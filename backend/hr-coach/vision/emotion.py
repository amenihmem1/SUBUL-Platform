            ...
            if isinstance(metadata.get("raw_emotion"), dict):
                ...
                for item in metadata.get("raw_predictions") or []:
                    ...
                    try:
                        ...
                        score = 0.0
                    raw_emotion_totals[canonical] = max(0.0, score * 100.0)
        if not raw_emotion_totals:
            continue
...
def update_visual_observations(
    ...
    *,
    ...
    if isinstance(mediapipe, dict):
        ...
        if isinstance(raw_objects, list):
            ...
            for item in raw_objects:
                ...
                if not label or label in {"person", "remote"}:
                    continue
                try:
    ...
    for result in provider_results:
        ...
        raw_distribution_weight = report_weight if report_weight > 0 else 1.0
        ...
        if isinstance(raw_distribution, list):
            for item in raw_distribution:
                ...
                try:
                    score = float(item.get("score", 0.0)) * 100.0 * raw_distribution_weight
                except Exception:
                    score = 0.0
            ...
            for canonical, score in normalized_scores.items():
                weighted_score = score * raw_distribution_weight
...
def build_visual_metrics(
    ...
    object_counts = {
        str(label): count
        for label, count in dict(payload.get("object_counts") or {}).items()
        if str(label).strip().lower() not in {"person", "remote"}
    }
    object_confidence_totals = {
        str(label): total
        for label, total in dict(payload.get("object_confidence_totals") or {}).items()
        if str(label).strip().lower() not in {"person", "remote"}
    }
