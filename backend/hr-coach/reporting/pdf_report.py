...
def _normalize_detected_objects(value: Any) -> List[Dict[str, Any]]:
    ...
    for item in value:
        ...
        if not label or label.lower() in {"person", "remote"}:
            continue
        try:
        ...
        objects.append(
            {
                "label": label,
                "count": count,
                "avg_confidence": _normalize_percent(item.get("avg_confidence")),
            }
        )
    return objects


def _normalize_proctoring_events(value: Any, fallback: str) -> List[str]:
    if not isinstance(value, list):
        return [fallback]
    items: List[str] = []
    for index, event in enumerate(value, start=1):
        ...
        when = str(event.get("time") or event.get("timestamp") or "").strip()
        label = message or reason or f"Alerte surveillance {index}"
        details = " - ".join(part for part in (reason if message else "", when) if part)
        items.append(f"{label}: {details}" if details else label)
    return items or [fallback]
...
def _draw_footer(c: canvas.Canvas, footer_title: str) -> None:
    ...
    c.drawString(PAGE_MARGIN, footer_y, "SUBUL RH Agent")
...
def _draw_competency_table(c: canvas.Canvas, report: Dict[str, Any], x: float, y: float, width: float, labels: Dict[str, Any]) -> float:
    ...
    current_y = y - 26
    for key in ("communication", "teamwork", "problem_solving", "motivation"):
...
def _resolve_report_language(payload: Dict[str, Any], report: Dict[str, Any]) -> str:
    ...
    if any(marker in summary for marker in ("the candidate", "overall", "strengths", "teamwork", "problem solving")):
        return "en"
    return "fr"
...
def build_candidate_report_pdf(session_id: str, payload: Dict[str, Any], output_dir: str | Path | None = None) -> Path:
    ...
    proctoring_events = _normalize_proctoring_events(
        payload.get("proctoring_events") or report.get("proctoring_events"),
        labels["proctoring_none"],
    )
    ...
    y = _draw_section_title(c, labels["hr_review"], PAGE_MARGIN, y)
    ...
    proctoring_height = _estimate_bullets_height(c, proctoring_events[:6], CONTENT_WIDTH) + 28
    y = _ensure_space(c, y, proctoring_height + 24)
    y = _draw_bullet_panel(
        c,
        title=labels["proctoring_title"],
        items=proctoring_events[:6],
        x=PAGE_MARGIN,
        y=y,
        width=CONTENT_WIDTH,
        fill=COLOR_PANEL_SOFT,
        bullet_color=COLOR_DANGER,
    ) - 4
...
def build_candidate_insights_pdf(session_id: str, payload: Dict[str, Any], output_dir: str | Path | None = None) -> Path:
    ...
    detected_objects = _normalize_detected_objects(visual_metrics.get("detected_objects"))
    object_tags = [
        f"{item['label']} x{item['count']}"
        for item in detected_objects[:8]
    ] or [labels["detected_objects_none"]]
    ...
    y = _ensure_space(c, y, 115)
    y = _draw_tag_panel(
        c,
        title=labels["detected_objects_title"],
        tags=object_tags,
        x=PAGE_MARGIN,
        y=y,
        width=CONTENT_WIDTH,
