...
def build_langchain_toolbox(
    *,
    ...
    def phase_strategy(
        ...
        strategies = {
            "INTRO": {
                "goal": "ouvrir l'entretien avec une question large, simple et chaleureuse",
                "natural_style": "salutation breve puis question unique sur parcours et motivation",
                "avoid": "pas de projet precis, pas d'entreprise precise, pas de repetition plus tard",
            },
            "BEHAVIOR": {
                "goal": "obtenir une situation vecue concrete et contextualisee",
                "natural_style": "relance ancree sur un projet ou une experience precise du CV, cite(e) explicitement, puis defi ou desaccord reel avec une forme neutre du type 'sur votre projet X, avez-vous deja ete confronte(e) a...'",
                "avoid": "pas d'intro, pas de techno, pas de formule vague comme 'sur l'un de vos projets', pas de question trop scolaire",
            },
            "SOFT": {
                "goal": "creuser communication, apprentissage, emotions ou recul sur l'exemple donne",
                "natural_style": "question de relance naturelle, plus reflexive que factuelle",
                "avoid": "pas de nouvelle intro, pas de reposer la meme question behavior",
            },
            "MOTIVATION": {
                "goal": "comprendre envie du poste, criteres de choix et projection future",
                "natural_style": "question directe sur motivation, alignement et attentes",
                "avoid": "pas de retour sur conflit passe, pas de nom/headline repete",
            },
            "FINAL": {
        ...
        payload = strategies.get(phase_name, strategies["BEHAVIOR"]).copy()
    ...
    return LangChainToolbox(
        context=StructuredTool.from_function(
            ...
            description="Prepare un contexte structure pour l'entretien RH a partir du CV, du contexte RAG et des derniers tours candidat.",
        ),
        ...
        phase_strategy=StructuredTool.from_function(
            func=phase_strategy,
            name="phase_strategy",
            description="Retourne l'objectif RH, le style naturel attendu et les interdits selon la phase courante.",
        ),
        turn_scoring=StructuredTool.from_function(
            func=score_single_turn,
            name="score_single_turn",
            description="Calcule les scores RH d'un seul tour candidat avec score_partial et skills.",
        ),
        scoring=StructuredTool.from_function(
            func=infer_interview_scores,
            name="infer_interview_scores",
            description="Recalcule les scores de communication, teamwork, problem solving et motivation.",
        ),
        report=StructuredTool.from_function(
            func=build_final_report,
            name="build_final_report",
            description="Genere le payload JSON du rapport RH final a partir des signaux d'entretien.",
        ),
