...
# ─────────────────────────────────────────────
# Azure OpenAI : JOB_AZURE_* (local .env) ou AZURE_OPENAI_* (docker-compose agent-job-search)
# ─────────────────────────────────────────────


def _azure_openai_settings() -> tuple[str, str, str]:
    endpoint = (os.getenv("JOB_AZURE_OPENAI_ENDPOINT") or os.getenv("AZURE_OPENAI_ENDPOINT") or "").strip()
    api_key = (os.getenv("JOB_AZURE_OPENAI_API_KEY") or os.getenv("AZURE_OPENAI_API_KEY") or "").strip()
    deployment = (
        os.getenv("JOB_AZURE_OPENAI_DEPLOYMENT_NAME")
...
def _validate_env() -> None:
    endpoint, api_key, _ = _azure_openai_settings()
    missing: list[str] = []
    if not endpoint:
        missing.append("endpoint (JOB_AZURE_OPENAI_ENDPOINT or AZURE_OPENAI_ENDPOINT)")
    if not api_key:
        missing.append("API key (JOB_AZURE_OPENAI_API_KEY or AZURE_OPENAI_API_KEY)")
    if missing:
        raise EnvironmentError(
            "Variables Azure OpenAI manquantes : " + ", ".join(missing) + "\n"
            "En Docker, docker-compose injecte AZURE_OPENAI_* depuis AZURE_OPENAI_JOB_SEARCH_* à la racine."
        )
...
def _deployment_name() -> str:
    return _azure_openai_settings()[2]
