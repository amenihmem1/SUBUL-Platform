...
from database import cosmos_user_to_cv_text as _cosmos_doc_to_cv_text
...
def generate_cv_with_llm(
    ...
    # ── Job tailoring block ───────────────────────────────────────────────────
    _job_block = ""
    if job_description or job_title or job_requirements:
        _job_block = f"""
JOB TAILORING RULES — THIS CV IS TARGETED AT A SPECIFIC JOB OFFER:
Job Title: {job_title}
Required Skills: {job_requirements[:600]}
Job Description: {job_description[:1000]}

TAILORING INSTRUCTIONS:
- Place the most job-relevant skills FIRST in every category
- Mirror the exact keywords from the job description in the PROFILE section
- If the candidate has ANY experience (academic, personal projects, volunteer, academic coursework) 
  that relates to the job requirements — include it under PROFESSIONAL EXPERIENCE or ACADEMIC PROJECTS
- For fresh graduates: expand the PROFILE to 5-6 strong sentences targeting this role
- Add a RELEVANT COURSEWORK section under EDUCATION if the candidate studied topics related to this job
- Expand bullet points in PROJECTS to explicitly mention technologies the job requires
- The job title line below the candidate's name MUST match the target role: {job_title}
"""

    system_prompt = f"""You are an expert CV writer specializing in ATS-optimized, job-tailored resumes for tech and data science roles.

{_language_instruction}

Your task: generate a COMPLETE, DENSE, FULL-PAGE professional CV — every section must be fully populated with no empty white space.
Combine: (1) candidate's original CV data, (2) platform learning data, (3) job tailoring if provided.
...
═══════════════════════════════════════════════════════
- First line = candidate full name only (ALL CAPS)
- Second line = target job title (plain text, NOT all caps)
...
- Use \\t (tab character) to right-align dates

═══════════════════════════════════════════════════════
CONTACT SECTION:
═══════════════════════════════════════════════════════
- All on ONE line separated by  |  symbol
- Include: email | phone (if available) | LinkedIn | location (if available)

═══════════════════════════════════════════════════════
PROFILE SECTION — MANDATORY, ALWAYS INCLUDE:
═══════════════════════════════════════════════════════
- ALWAYS write this section — even for fresh graduates
- 4-6 strong sentences targeting the specific job role
{domain_instruction}- Lead with the candidate's degree + strongest technical skills
- Mirror keywords from the job description (Power BI, ETL, DAX, SQL, etc. if relevant)
- Mention hands-on experience from academic projects, personal work, ETL automation, dashboards
- End with career objective aligned to the target role
- Example for fresh graduate: "Recent Data Science graduate with hands-on experience in ETL 
  automation, Power BI dashboards, and Python data pipelines. Proficient in Power Query, DAX, 
  SQL, and Scikit-learn with proven ability to clean and transform fragmented data into 
  actionable insights. Built end-to-end analytics solutions as academic projects using 
  TensorFlow, Tableau and PostgreSQL. Seeking to bring strong analytical skills and data 
  storytelling capabilities to a BI & Data Analyst role."
...
- Below degree: add 3-5 bullet points of RELEVANT COURSEWORK matching the job:
  - List courses/modules that match job requirements (Statistics, Data Visualization, Databases, ML, etc.)
  - Format: "- Relevant coursework: Statistics, Machine Learning, Database Systems, Data Visualization, Business Analytics"
- Add any academic honours, GPA, rank if mentioned in source

═══════════════════════════════════════════════════════
PROFESSIONAL EXPERIENCE SECTION:
═══════════════════════════════════════════════════════
- Include ONLY if real work/internship experience exists in the source CV
- Format: **Job Title | Company Name**\tDate
- 3-5 bullet points per role using strong action verbs (Designed, Built, Automated, Analyzed, etc.)
- Quantify impact wherever possible: "reduced reporting time by 40%", "automated 15 manual reports"
- If NO professional experience exists → OMIT this section entirely (do NOT write "N/A" or placeholder)

═══════════════════════════════════════════════════════
ACADEMIC PROJECTS / PROJECTS SECTION — CRITICAL FOR FRESH GRADUATES:
═══════════════════════════════════════════════════════
- For fresh graduates with no work experience, this section is ESSENTIAL — it must be rich and detailed
- Include ALL projects from the CV source data
- Include ALL platform labs as individual project entries
- For EACH project: write 2-3 bullet points explaining WHAT was built, HOW (technologies), and IMPACT/RESULTS
- Explicitly mention technologies relevant to the target job
- Format:
  **Project Name** — Year
  - Built [what] using [technologies] achieving [result/metric]
  - Implemented [feature] with [tool] to [solve what problem]
  - Demonstrated [skill relevant to job description]
- NEVER merge two projects into one entry
- If NO projects exist at all → OMIT this section

═══════════════════════════════════════════════════════
SKILLS SECTION — ALWAYS INCLUDE, ALWAYS FULL:
═══════════════════════════════════════════════════════
- Combine ALL skills from: (a) original CV, (b) platform inferred skills, (c) skills implied by the job
- Group into meaningful categories — use the categories that best fit:
    **Business Intelligence & Visualization:** Power BI, DAX, Power Query, Tableau, Looker, Excel, KPIs, ...
    **Data Engineering & ETL:** ETL, Power Query, Data Pipelines, Data Cleaning, Data Transformation, ...
    **Programming & Databases:** Python, R, SQL, PostgreSQL, MySQL, Pandas, NumPy, ...
    **Machine Learning & Analytics:** Scikit-learn, TensorFlow, Regression, Classification, Statistics, ...
    **Cloud & DevOps:** AWS, Azure, Docker, Git, Jupyter Notebook, ...
    **Tools & Productivity:** Excel (Advanced), PowerPoint, Confluence, JIRA, ...
- ONLY create a category if it has real skills to list
- Every skill from the source CV MUST appear in one category
- Put job-critical skills in the FIRST category

═══════════════════════════════════════════════════════
CERTIFICATIONS & TRAINING SECTION:
═══════════════════════════════════════════════════════
- Include ALL certifications from the CV + platform data
- Sort by date descending (most recent first)
- Format: - **Certification Name** · Issued by Organization — Date
- OMIT this section ONLY if zero certifications exist

═══════════════════════════════════════════════════════
LANGUAGES SECTION:
═══════════════════════════════════════════════════════
- Include ALL languages found anywhere in the source data
- Format: Language: Level | Language: Level (e.g. "English: Fluent | French: Native")
- OMIT only if truly no language information exists anywhere

═══════════════════════════════════════════════════════
ABSOLUTE RULES — NEVER VIOLATE:
═══════════════════════════════════════════════════════
1. NEVER write placeholder text: 'Degree Title', 'Institution name', 'N/A', 'TBD', etc.
2. NEVER invent work experience, companies, or employment dates
3. NEVER invent certifications or awards
4. CAN and SHOULD expand academic projects with richer bullet points based on the skills listed
5. CAN and SHOULD add relevant coursework bullets under education
6. NEVER use the word 'Category' as a skill category label
7. If the CV is clearly an empty template → respond only with: ERROR_TEMPLATE
8. Target length: 60-80 lines — the CV must look FULL, not half-empty
9. The right column (Experience/Projects) must never be empty — always populate with projects
"""
    ...
    # ── Build job context block for user prompt ──────────────────────────────
    _job_context_for_prompt = ""
    if job_description or job_title or job_requirements:
        _job_context_for_prompt = f"""
TARGET JOB OFFER (tailor every section to this role):
---
Job Title: {job_title}
Required Skills: {job_requirements[:600]}
Full Job Description: {job_description[:1200]}
---
"""

    user_prompt = f"""CANDIDATE'S ORIGINAL CV DATA:
---
{cv_text}
---
...
IMPORTANT: Generate a COMPLETE, FULL-PAGE CV. Every section must be rich and populated.
For fresh graduates: expand PROJECTS with detailed bullets, add RELEVANT COURSEWORK under education,
write a strong 5-sentence PROFILE targeting the job role. Do NOT leave the right column empty.

Generate the complete ATS-optimized CV now."""

    log.info("📄 Sending CV to GPT for rewrite (%d chars, job=%s, skipping: %s)...",
             len(cv_text), job_title[:40] if job_title else "none", skipped_sections)
    resp = client.chat.completions.create(
        model=AZURE_DEPLOYMENT,
        messages=[
...
async def boost_cv(
    ...
    # ── 3. ATS score BEFORE ───────────────────────────────────────────────────
    # Si le frontend a déjà calculé le score (via /api/ats-score), on l'utilise
    # directement pour éviter un double appel GPT-4o non-déterministe.
    try:
        extra_pre = json.loads(extra_data) if extra_data and extra_data.strip() not in ("{}", "") else {}
    except Exception:
        extra_pre = {}

    ats_score_before_locked       = extra_pre.get("ats_score_before_locked", False)
    ats_score_before_from_frontend = extra_pre.get("ats_score_before", None)

    cv_sections_before = parse_cv_sections(raw_text)

    if ats_score_before_locked and ats_score_before_from_frontend is not None:
        # Utiliser le score déjà calculé par /api/ats-score — cohérence garantie
        score_before = {
            "total": int(ats_score_before_from_frontend),
            "max": 100,
            "breakdown": calculate_ats_score(cv_sections_before, raw_text, domain_keywords=domain_keywords)["breakdown"],
        }
        log.info("[boost] ats_score_before from frontend (locked): %d", score_before["total"])
    else:
...
async def enhance_cv_for_job(
    ...
    job_description_text: str = Form(""),   # optional: full JD text passed directly
    job_title_text:       str = Form(""),   # optional: job title passed directly
    job_requirements_text:str = Form(""),   # optional: requirements passed directly
):
    """
    ...
    # Start with directly provided values (fallback if DB lookup fails)
    job_title       = job_title_text.strip()
    job_description = job_description_text.strip()
    job_requirements = job_requirements_text.strip()
    ...
    if job_id:
        try:
            ...
            if rows:
                job = rows[0]
                # DB values take priority over directly passed values
                job_title       = job.get("title", "") or job_title
                job_description = job.get("description", "") or job_description
                job_requirements = (job.get("must_have", "") or job.get("requirements", "")) or job_requirements
                log.info("✓ Job found in DB: %s", job_title)
            else:
                log.warning("⚠ Job not found in DB for url: %s — using directly passed data", job_id[:60])
        except Exception as e:
