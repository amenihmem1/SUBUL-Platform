...
# PyMuPDF availability flag (used to gracefully disable PDF image extraction)
FITZ_OK = fitz is not None

import pdfplumber
import docx as docx_reader

from section_utils import parse_cv_sections, infer_section_presence_heuristics
...
def _should_retry_pdf_with_vision(text: str) -> bool:
    """Heuristics see education/experience content but strict header parse failed (common with 2-col PDFs)."""
    if len(text) < 400 or len(text.split()) < 80:
        return False
    sec = parse_cv_sections(text)
    h = infer_section_presence_heuristics(text)
    return (h["experience"] and not sec.get("experience")) or (h["education"] and not sec.get("education"))
...
def extract_text_from_pdf(file_bytes: bytes) -> str:
    ...
    try:
        ...
        if not _is_thin(text):
            if _should_retry_pdf_with_vision(text):
                imgs = _pdf_to_images_b64(file_bytes)
                if imgs:
                    vt = _vision_extract(imgs)
                    if vt and len(vt) > 100:
                        sec_p = parse_cv_sections(text)
                        sec_v = parse_cv_sections(vt)
                        score_p = sum(1 for k in ("experience", "education") if sec_p.get(k))
                        score_v = sum(1 for k in ("experience", "education") if sec_v.get(k))
                        if score_v > score_p or (score_v >= score_p and len(vt) >= len(text) * 0.85):
                            log.info(
                                "✓ PDF: vision structure upgrade (%d chars, sections exp/edu: %d→%d)",
                                len(vt), score_p, score_v,
                            )
                            return vt
