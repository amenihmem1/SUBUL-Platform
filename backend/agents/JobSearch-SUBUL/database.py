...
def _get_cosmos_client() -> str:
    endpoint = os.getenv("AZURE_COSMOS_ENDPOINT", "https://cosmosdbtutot.documents.azure.com:443/").strip()
    key      = os.getenv("AZURE_COSMOS_KEY", "HIDGG5hzdItrc78CAxzZOyOwBIiqzhsaQuTCh4BH0V5wpqGKY6T5TbBp7oQi5N8VBAuxpPgnEE0UACDbH8fLEw==").strip()
...
async def upsert_user(user_id: int, cv_structured: dict) -> bool:
    ...
    doc = {
        ...
        "languages":  cv_structured.get("languages") or existing.get("languages"),
        "certifications": cv_structured.get("certifications") or existing.get("certifications"),
        "labs":       cv_structured.get("labs") or existing.get("labs"),
        "quiz_data":  cv_structured.get("quiz_data") or existing.get("quiz_data"),
        "cv_raw_text": cv_structured.get("cv_raw_text", "") or cv_structured.get("cv_text", ""),
...
async def get_user(user_id: int) -> Optional[dict]:
    """Fetch user profile from Cosmos DB users container."""
    container_users, _, _ = _get_containers()
    try:
        logger.info(f"[db] get_user: attempting to read user_id={user_id} from Cosmos")
        row = await asyncio.to_thread(container_users.read_item, item=str(user_id), partition_key=str(user_id))
        logger.info(f"[db] get_user: ✓ found user_id={user_id} — returning profile")
        ...
        result = {
            ...
            "years_experience": row.get("years_exp") or row.get("years_experience") or row.get("experience_years"),
        ...
        logger.warning(f"[db] get_user: ✗ user_id={user_id} not found in Cosmos users container")
        return None
    except Exception as e:
        logger.error(f"[db] get_user: ✗ error reading user_id={user_id}: {type(e).__name__}: {e}")
        return None
...
def cosmos_user_to_cv_text(doc: dict) -> str:
    """Rebuild CV plain text from structured Cosmos user fields (when cv_raw_text is empty)."""
    lines: list[str] = []
    ...
    role_meta = [p for p in [doc.get("role"), doc.get("seniority"), doc.get("industry")] if p]
    if role_meta:
        lines.append("\nROLE")
        lines.append(" · ".join(str(x) for x in role_meta))
    ye = doc.get("years_experience") or doc.get("years_exp")
    if ye:
        lines.append(f"Years of experience: {ye}")
    if doc.get("summary"):
        lines.append("\nPROFILE")
    ...
    if bullets:
        lines.append("\nEXPERIENCE")
        lines.append(bullets if isinstance(bullets, str) else str(bullets))
