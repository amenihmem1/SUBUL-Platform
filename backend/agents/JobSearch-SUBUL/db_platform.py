...
def _get_cosmos_user(user_id: str) -> dict | None:
    ...
    endpoint       = os.getenv("AZURE_COSMOS_ENDPOINT", "https://cosmosdbtutot.documents.azure.com:443/")
    key            = os.getenv("AZURE_COSMOS_KEY", "HIDGG5hzdItrc78CAxzZOyOwBIiqzhsaQuTCh4BH0V5wpqGKY6T5TbBp7oQi5N8VBAuxpPgnEE0UACDbH8fLEw==")
...
# ── CosmosDB helper: create user if absent (use from fetch_platform_data)
# ───────────────────────────────────────────────────────────────────────────
def _create_cosmos_user(user_id: str) -> bool:
    ...
    try:
        ...
        col.upsert_item(payload)
        log.info("➕ CosmosDB created new user %s", user_id)
        return True
    except Exception as e:
        log.error("Failed to create CosmosDB user %s: %s", user_id, e)
        return False
...
def _fetch_labs(cur, uid: int):
    try:
        cur.execute("""
            SELECT l.id, l.title, lp.time_spent, lp.started_at, lp.completed_at, lp.is_completed
            FROM   lab_progress lp JOIN labs l ON l.id = lp.lab_id
            WHERE  lp.user_id = %s
            ORDER  BY lp.completed_at DESC NULLS LAST, lp.started_at DESC
        """, (uid,))
        rows = cur.fetchall()
        result = []
        for i, row in enumerate(rows):
            ...
            result.append({
                ...
                "score":        score,
                "started_at":   _fmt_date(row[3]),
                "completed_at": _fmt_date(row[4]),
...
def fetch_platform_data(user_id: str) -> dict:
    ...
    if not doc:
        log.error("CosmosDB user lookup failed for user %s", uid)
        return {"status": "db_error", "quiz": None, "labs": [], "certifications": []}
    
    if doc.get("_new"):
        log.info("User %s is new — no data yet", uid)
        created = _create_cosmos_user(user_id)
        if created:
            log.info("User %s created in CosmosDB after missing lookup", uid)
        else:
            log.warning("Failed to create user %s in CosmosDB", uid)
...
def fetch_recommendations(user_id: str, quiz_domain: str) -> dict:
    ...
    try:
        ...
        # Recommandations labs : proposer des labs non commencés pour l'utilisateur
        cur.execute("""
        ...
        all_labs = [
            ...
            for r in lab_rows if r[1]
        ]

        return {"certifications": all_certs[:6], "labs": all_labs}
    except Exception as e:
