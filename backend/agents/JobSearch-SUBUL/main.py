...
_AGENTS_DIR = str(BASE_DIR.parent)
if _AGENTS_DIR not in sys.path:
    sys.path.insert(0, _AGENTS_DIR)
from shared.metrics import AgentMetrics as _AgentMetrics, add_metrics_endpoint, health_response
_metrics = _AgentMetrics("job-search")

# Ensure local agent modules are importable when starting from backend root path
if str(BASE_DIR) not in sys.path:
    sys.path.insert(0, str(BASE_DIR))

_cv_booster_path = BASE_DIR.parent / 'CV_Booster_Agent'
if str(_cv_booster_path) not in sys.path:
    sys.path.insert(0, str(_cv_booster_path))
from fastapi import FastAPI, HTTPException
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import HTMLResponse, StreamingResponse
from openai import AsyncAzureOpenAI
from pydantic import BaseModel, field_validator
from enhance_cv import enhance_router

from database import (
    init_db, close_pool, get_user, get_jobs_for_user, cosmos_user_to_cv_text,
)
...
from ats_extractor  import prepare_cv_text
from ats_structurer import structure_cv, structure_jd
from ats_scorer     import compute_ats_score

# Import propre du router save_cv (au lieu de 'import save_cv' à la fin)
from save_cv import router as cv_workflow_router
logging.basicConfig(level=logging.INFO, format="%(levelname)s  %(message)s")
logger = logging.getLogger(__name__)

# Cache en mémoire pour rendre le scoring reproductible sur identique CV/JD
_STRUCTURED_CV_CACHE: dict[str, dict] = {}
_STRUCTURED_JD_CACHE: dict[str, dict] = {}
...
async def job_search_health():
    return health_response("job-search")

app.include_router(chat_router)
app.include_router(scraping_router)
app.include_router(jobs_router)
# CV booster endpoints (API URL expected by frontend). Prefix ensures /api/cv/extract-cv, /api/cv/save-cv, etc.
app.include_router(enhance_router, prefix="/api/cv")
# CV workflow endpoints under /api/cv for Nest proxy compatibility.
app.include_router(cv_workflow_router, prefix="/api/cv")
# Endpoints historiques (ex: /save-cv à la racine) kept for backward compatibility.
app.include_router(cv_workflow_router)
...
async def startup():
    # Do not block HTTP forever on Cosmos or slow networks — uvicorn only accepts
    # connections after this handler returns.
    try:
        await asyncio.wait_for(init_db(), timeout=90.0)
    except asyncio.TimeoutError:
        logger.error("[startup] init_db timed out after 90s; API will start degraded (Cosmos unreachable?)")
    except Exception as e:
        logger.exception("[startup] init_db failed; API will start degraded: %s", e)
    logger.info("[startup] Fixed user_id=1 ")
    try:
        await asyncio.wait_for(_refresh_market_analysis(), timeout=120.0)
    except asyncio.TimeoutError:
        logger.error("[startup] market refresh timed out after 120s")
    except Exception as e:
        logger.warning("[startup] market refresh error: %s", e)
...
class ATSRequest(BaseModel):
    user_id: str = ""
    job_description: str = ""  # texte de la JD
...
async def _user_report_matches(user_id: int, prof: CandidateProfile, top_n: int = 20) -> list:
    ...
    for j in jobs_raw:
        ...
        # Si skills_req est vide, essayer de reconstruire depuis gap_missing + gap_matched
        if not req_skills:
            gap_miss = list(j.get("gap_missing") or [])
            # Les skills que le user a sont dans prof.skills — les croiser avec le contenu du job
            ...
            # Skills du candidat qui apparaissent dans le texte du job = matched
            matched = [s for s in prof.skills if s.lower() in job_text]
            missing = gap_miss
        else:
            matched = [s for s in req_skills if s.lower() in prof_skills_lower]
            missing = list(j.get("gap_missing") or [])

        # Fallback final : si toujours vide, croiser skills du candidat avec texte du job
        if not matched:
            job_text = " ".join(filter(None, [
                j.get("title", ""),
                j.get("description", ""),
...
async def api_ats_score(data: ATSRequest):
    """
    Calcule le score ATS entre le CV de l'utilisateur et une description de poste.
    Le cv_text est chargé depuis CosmosDB (sauvegardé lors du scan).
    """
    if not data.user_id:
        raise HTTPException(400, "user_id requis")
    if not data.job_description.strip():
        raise HTTPException(400, "job_description requis")

    # 1. Charger cv_text depuis CosmosDB
    user = await get_user(int(data.user_id))
    if not user:
        raise HTTPException(404, "Utilisateur non trouvé")
    
    cv_text = (user.get("cv_raw_text") or "").strip()
    # Upload / CV booster often fill structured fields only; scan saves cv_raw_text.
    if len(cv_text) < 50:
        cv_text = cosmos_user_to_cv_text(user).strip()
    if len(cv_text) < 50:
        raise HTTPException(
            400,
            "CV non disponible. Téléversez votre CV ou lancez une recherche d'emploi (scan) pour enregistrer le texte du CV.",
        )

    # 2. Préparer le texte CV
    try:
        cv_clean = prepare_cv_text(cv_text)
    except ValueError as e:
        raise HTTPException(400, str(e))

    # 3. Structurer CV + JD via GPT-4o (synchrone → thread) avec cache déterministe
    cv_key = _hash_text(cv_clean)
    jd_key = _hash_text(data.job_description)

    cv_data = _STRUCTURED_CV_CACHE.get(cv_key)
    jd_data = _STRUCTURED_JD_CACHE.get(jd_key)

    try:
        if cv_data is None:
            cv_data = await asyncio.to_thread(structure_cv, cv_clean)
            _STRUCTURED_CV_CACHE[cv_key] = cv_data
        if jd_data is None:
            jd_data = await asyncio.to_thread(structure_jd, data.job_description)
            _STRUCTURED_JD_CACHE[jd_key] = jd_data
    except ValueError as e:
        raise HTTPException(500, f"Erreur structuration GPT : {e}")
    except ConnectionError as e:
        raise HTTPException(503, f"Azure OpenAI indisponible : {e}")

    # 4. Calculer le score ATS
    try:
        result = await asyncio.to_thread(compute_ats_score, cv_data, jd_data)
    except Exception as e:
        raise HTTPException(500, f"Erreur scoring ATS : {e}")
    ...
    return {
        ...
        "skills_matched":   result["skills_result"]["matched"],
        "cv_data":          cv_data,
        "jd_data":          jd_data,
    }