...
_AGENTS_DIR = str(_Path(__file__).resolve().parent.parent)
if _AGENTS_DIR not in _sys.path:
    _sys.path.insert(0, _AGENTS_DIR)
from shared.metrics import AgentMetrics as _AgentMetrics, add_metrics_endpoint, health_response
_metrics = _AgentMetrics("cv-booster")
from fastapi import APIRouter, FastAPI, UploadFile, File, Form, Path as FPath
...
# Ensure cv_extraction from JobSearch-SUBUL is importable in cross-agent mode.
base_path = Path(__file__).resolve().parent
jobsearch_path = base_path.parent / 'JobSearch-SUBUL'
if str(jobsearch_path) not in sys.path:
    sys.path.insert(0, str(jobsearch_path))

load_dotenv(Path(__file__).parent / ".env")   # ← AJOUTER CES 3 LIGNES ICI

# ── Azure OpenAI (TES variables .env) ────────────────────────────────────────
# Use `or` instead of getenv default so empty-string env vars (set by docker-compose
# when AZURE_OPENAI_CV_BOOSTER_ENDPOINT is absent) fall back to the baked-in defaults.
AZURE_DEPLOYMENT = (
    os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME") or
    os.getenv("AZURE_OPENAI_DEPLOYMENT") or
    "gpt-4o-mini"
)
_oai_endpoint = os.getenv("AZURE_OPENAI_ENDPOINT") or "https://cvazureopenai.openai.azure.com/"
_oai_key      = os.getenv("AZURE_OPENAI_API_KEY")   or "Eettc4PUxKZRrmWLehzjtxrmaKuv8ExnqrQhmJL4bap5cppyTUt5JQQJ99CCACHYHv6XJ3w3AAABACOGdy2W"
_oai_version  = os.getenv("AZURE_OPENAI_API_VERSION") or "2024-08-01-preview"
try:
    client = AzureOpenAI(
        api_version    = _oai_version,
        azure_endpoint = _oai_endpoint,
        api_key        = _oai_key,
    )
except Exception as _oai_err:
    log.warning("⚠️  AzureOpenAI init failed (%s). CV LLM features will return errors.", _oai_err)
    client = None
...
def get_domain_keywords(domain: str) -> list:
    ...
    try:
        ...
        _metrics.record_openai_usage(getattr(resp, "usage", None), AZURE_DEPLOYMENT, _time.perf_counter() - _t0)
        ...
        _metrics.record_openai_error(AZURE_DEPLOYMENT, "error")
...
def generate_cv_with_llm(
    ...
    _cv_language = language if language in ("French", "English") else ("French" if _fr_count >= 4 else "English")
    ...
    # ── Build job-targeting block ─────────────────────────────────────────────
    _job_targeting_block = ""
    if job_title or job_description or job_requirements:
        _job_targeting_block = f"""
═══════════════════════════════════════════════
JOB-TARGETING INSTRUCTIONS — READ CAREFULLY
═══════════════════════════════════════════════
You are tailoring this CV SPECIFICALLY for the following role. This is the #1 priority.
...
MANDATORY JOB-TAILORING RULES:
1. PROFILE SECTION: Write 4-5 sentences directly referencing the job title ("{job_title}") and keywords from the JD above. Mirror the employer's language.
2. SKILLS SECTION: Put all skills that appear in the job description FIRST in each category. Add any JD-required skills the candidate has (Power BI, DAX, Power Query, SQL, Excel, KPI, ETL, data pipeline, data governance, data storytelling, Power Pivot) even if not explicitly listed in original CV — only if genuinely supported by their background.
3. EXPERIENCE SECTION: For each bullet, lead with the accomplishments most relevant to this role. Use verbs from the JD: "designed", "built", "maintained", "automated", "standardized", "transformed", "delivered", "optimized".
4. KEYWORDS: Embed these exact JD keywords naturally throughout the CV: Power BI, DAX, Power Query, ETL, KPI, data pipeline, data governance, data storytelling, stakeholders, actionable insights, data accuracy, automation, SQL.
5. DO NOT invent experience or achievements not in the original CV. Only use what exists + reframe it for this role.
6. ATS TARGET: This CV must score above 75/100 ATS for this specific role.
═══════════════════════════════════════════════
"""
    ...
    system_prompt = f"""You are an expert CV writer specializing in ATS-optimized resumes for tech and data roles.

{_language_instruction}

{_job_targeting_block}

Your task: generate a COMPLETE, FULL-PAGE, professional, ATS-friendly CV combining:
...
PAGE FULLNESS RULE — CRITICAL:
- The CV MUST fill a complete A4 page with NO large blank white spaces.
- Aim for 65-80 lines of content (not 50).
- If the candidate has little experience, EXPAND other sections:
  • Profile: 5-6 sentences (not 3-4)
  • Skills: list ALL skills with subcategories, write 2-3 lines per category
  • Education: add relevant coursework, thesis topic, academic projects
  • Projects: if no work experience, describe academic/personal projects in detail (3-4 bullets each)
  • Add an "Objective" or "Career Goal" paragraph if profile is short
- NEVER leave the right side of the CV blank. Fill every section fully.
...
PROFILE SECTION:
- 5-6 strong sentences: who you are, what you do, what you bring, career goal
...
- For entry-level candidates: write a compelling narrative about their academic strengths,
  technical skills, and what value they will bring — make it 5+ sentences

EDUCATION SECTION:
- Format: **Degree Title**\t2020 - 2023
- Next line: Institution name, City
- Add 2-3 lines of relevant coursework or academic highlights
...
Order all experiences by date in descending order (most recent first, oldest last)
 ...
 Bullet points starting with "- " using strong action verbs — MINIMUM 4 bullets per role
...
- Format each project/lab as:
  **Project Name** — Year
  2-3 bullet lines: what was built, which technologies, and the outcome/impact.
...
- Write a MINIMUM of 4 category lines to ensure the skills section is substantial
...
- Aim for 65-80 lines total — FILL THE PAGE
...
def generate_docx_modern(parsed: dict) -> bytes:
    ...
    # Fill remaining white space: add extra spacing between sections and a career objective footer
    _rcell_content_keys = [k for k in ["experience","education","projects"] if parsed["sections"].get(k)]
    if len(_rcell_content_keys) < 3:
        # Right column is sparse — add generous spacing after each section
        for p in rcell.paragraphs:
            if p.paragraph_format.space_after and p.paragraph_format.space_after.pt and p.paragraph_format.space_after.pt < 8:
...
async def boost_cv(
    ...
    if file is not None:
        ...
        # No file uploaded — fetch stored CV text from Cosmos by user_id
        content  = b""
        filename = "cv.txt"
        if user_id:
            try:
                ...
                raw = str(doc.get("cv_raw_text", "") or "").strip()
                if not raw:
                    raw = _cosmos_doc_to_cv_text(doc).strip()
                if raw:
                    content = raw.encode("utf-8")
                    log.info("📥 Fetched stored CV from Cosmos for user %s (%d bytes)", user_id, len(content))
            except Exception as _e:
                log.warning("Could not fetch stored CV from Cosmos for user %s: %s", user_id, _e)
        if not content:
            from fastapi import HTTPException as _HTTPException
            raise _HTTPException(status_code=422, detail="No CV file provided and no stored CV found. Please upload your CV first.")
    ...
    # Optional job context (if provided by frontend) — must never crash CV saving.
    job_title = str(extra.get("job_title", "") or "").strip() if isinstance(extra, dict) else ""
    job_description = str(extra.get("job_description", "") or "").strip() if isinstance(extra, dict) else ""
    job_requirements = str(extra.get("job_requirements", "") or "").strip() if isinstance(extra, dict) else ""

    # Precompute document language for job-context instructions (DOCX generation does its own detection later).
    ...
    try:
        ...
        if job_title or job_description or job_requirements:
            log.info("🎯 Job-targeting active: %s", job_title)
    ...
    log.info("🔑 boost-cv process done (no CosmosDB write for boosted CV)")
    # Dans cette route /boost-cv, on ne modifie plus le document utilisateur existant.
    # L'image générée est renvoyée au client sans persistance dans Cosmos.
...
# ═════════════════════════════════════════════════════════════════════════════
# ENDPOINT 0 — POST /extract-cv
# Lightweight extraction step for the frontend.
# Returns JSON expected by `CVBoosterApp.handleFile()`.
# ═════════════════════════════════════════════════════════════════════════════
@enhance_router.post("/extract-cv")
async def extract_cv(
    ...
    # The frontend expects these exact keys for step 2.
...
# ═════════════════════════════════════════════════════════════════════════════
# ENDPOINT 1 — POST /save-cv
# Extracts CV data, runs LLM enrichment, saves to CosmosDB, returns metadata.
# Unlike /boost-cv (which returns DOCX only), this persists the structured CV
# to CosmosDB so /document and /cv-status return current data.
# ═════════════════════════════════════════════════════════════════════════════
@enhance_router.post("/save-cv")
async def save_cv(
    file: UploadFile = File(...),
    user_id: str = Form(""),
    quiz_data: str = Form("null"),
    ...
    try:
        labs_obj = json.loads(labs_data) if labs_data and labs_data.strip() else []
    except Exception:
        labs_obj = []
    try:
        certs_obj = json.loads(certs_data) if certs_data and certs_data.strip() else []
    except Exception:
        certs_obj = []
    try:
        quiz_obj = None if (not quiz_data or quiz_data in ("null", "", "None")) else json.loads(quiz_data)
    except Exception:
        quiz_obj = None

    include_labs_ids = []
    for o in labs_obj if isinstance(labs_obj, list) else []:
        if isinstance(o, dict) and o.get("id") is not None:
            include_labs_ids.append(str(o.get("id")))

    include_certs_ids = []
    for o in certs_obj if isinstance(certs_obj, list) else []:
        if isinstance(o, dict) and o.get("id") is not None:
            include_certs_ids.append(str(o.get("id")))

    include_quiz_str = "true" if quiz_obj else "false"

    # Run boost_cv to get enriched CV data
    boost_response = await boost_cv(
        file=file,
        cv_format="ats",
        include_quiz=include_quiz_str,
        include_labs=json.dumps(include_labs_ids),
        include_certs=json.dumps(include_certs_ids),
    ...
    # Persist structured CV data to CosmosDB so /document and /cv-status return current data
    if user_id and isinstance(boost_response, StreamingResponse):
        try:
            # Extract parsed CV data from response headers
            parsed_cv_str = boost_response.headers.get("X-Parsed-CV", "")
            ats_score_before = boost_response.headers.get("X-ATS-Score-Before", "0")
            ats_score_after = boost_response.headers.get("X-ATS-Score-After", "0")
            domain = boost_response.headers.get("X-Domain", "")

            if parsed_cv_str:
                parsed = json.loads(parsed_cv_str)
                sections = parsed.get("sections", {})
                cv_structured = {
                    "first_name": parsed.get("name", "").split()[0] if parsed.get("name") else "",
                    "last_name": " ".join(parsed.get("name", "").split()[1:]) if parsed.get("name") else "",
                    "role": parsed.get("job_title", "") or domain,
                    "seniority": "",
                    "years_experience": "",
                    "industry": domain,
                    "education": "\n".join(sections.get("education", [])) if sections.get("education") else "",
                    "skills": "\n".join(sections.get("skills", [])) if sections.get("skills") else "",
                    "summary": "\n".join(sections.get("summary", [])) if sections.get("summary") else "",
                    "bullets": "",
                    "ats_score_before": int(ats_score_before) if ats_score_before.isdigit() else 0,
                    "ats_score_after": int(ats_score_after) if ats_score_after.isdigit() else 0,
                    "cv_raw_text": parsed_cv_str,
                }
                save_cv_to_cosmos(user_id, cv_structured)
                log.info("✅ Saved enriched CV to CosmosDB for user %s", user_id)
        except Exception as e:
            log.error("❌ Failed to save CV to CosmosDB for user %s: %s", user_id, e)
...
@enhance_router.post("/store-user")
async def store_user(user_id: str = Form(...)):
    """
    Create/update a minimal learner profile doc in Cosmos.
    """
    ...
    try:
        ...
        doc = {
            ...
            "seniority": existing.get("seniority", ""),
            "years_exp": existing.get("years_exp", "0"),
            "industry": existing.get("industry", ""),
            "education": existing.get("education", ""),
            "skills": existing.get("skills", ""),
            "summary": existing.get("summary", ""),
            "bullets": existing.get("bullets", ""),
            "languages": existing.get("languages", ""),
            "cv_raw_text": existing.get("cv_raw_text", ""),
            "cv_file_name": existing.get("cv_file_name", ""),
        ...
        log.error("❌ /store-user failed: %s", e)
        return JSONResponse({"error": "Database save failed"}, status_code=500)


@enhance_router.get("/cv-status/{user_id}")
async def cv_status(user_id: str = FPath(...)):
    """
    Canonical CV status for learner emploi page.
    """
    if not user_id or user_id == "user_default":
        return JSONResponse({"hasCv": False, "status": "missing"})
    ...
    try:
        ...
        return JSONResponse({"hasCv": False, "status": "error"}, status_code=500)

    raw_text = str(doc.get("cv_raw_text", "") or "").strip()
    fallback_text = _cosmos_doc_to_cv_text(doc).strip()
    has_structured = any(
        str(doc.get(k, "") or "").strip() for k in ("skills", "summary", "education", "role")
    )
    has_cv = len(raw_text) >= 50 or len(fallback_text) >= 50 or has_structured

    skills_value = str(doc.get("skills", "") or "")
    skills_count = len([s for s in skills_value.split(",") if s.strip()]) if skills_value else 0

    return JSONResponse({
        "hasCv": has_cv,
        "status": "ready" if has_cv else "missing",
        "lastUploadedAt": doc.get("updated_at") or doc.get("_ts"),
        "fileName": doc.get("cv_file_name"),
        "cvPreview": {
...
_MAX_CV_TEXT_CHARS = 100_000


@enhance_router.get("/document/{user_id}")
async def get_cv_document(user_id: str = FPath(...)):
    """
    Read-only CV payload for learner "View CV" (Cosmos users container).
    """
    if not user_id or user_id == "user_default":
        return JSONResponse({"exists": False, "hasContent": False}, status_code=404)
    ...
    try:
        ...
        log.error("❌ /document read failed for user %s: %s", user_id, e)
        return JSONResponse({"error": "Failed to load CV document"}, status_code=500)
    ...
    has_content = len(raw_text) >= 50 or len(fallback_text) >= 50 or has_structured
    if not has_content:
        return JSONResponse({"exists": True, "hasContent": False}, status_code=404)

    full_text = raw_text if len(raw_text) >= 50 else fallback_text
    truncated = len(full_text) > _MAX_CV_TEXT_CHARS
    if truncated:
        full_text = full_text[:_MAX_CV_TEXT_CHARS]
    ...
    return JSONResponse({
        ...
        "seniority": doc.get("seniority") or "",
        "years_exp": doc.get("years_exp") or "",
        ...
        "cv_file_name": doc.get("cv_file_name") or "",
        "updated_at": doc.get("updated_at") or doc.get("_ts") or "",
        "full_text": full_text,
...
"""
    ...
    # ── 6. Generate enhanced CV with job context injected via LLM params ─────────
    try:
...
# ── Standalone ASGI app (Docker: `uvicorn enhance_cv:app`; same mount as JobSearch main.py)
app = FastAPI(title="CV Booster Agent", version="1.0")
app.include_router(enhance_router, prefix="/api/cv")
add_metrics_endpoint(app)

@app.get("/health", tags=["Monitoring"])
async def cv_health():
    return health_response("cv-booster")