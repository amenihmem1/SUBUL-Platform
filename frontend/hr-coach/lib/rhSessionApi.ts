...
export async function fetchSession(sessionId: string, options?: { includeInsights?: boolean }) {
  ...
  const res = await fetch(rhApiPath(`/api/rh/session/${encodeURIComponent(sessionId)}${query}`), {
    method: "GET",
    cache: "no-store",
  });
...
export async function updatePreferredInputMode(sessionId: string, preferredInputMode: InputMode) {
  const res = await fetch(rhApiPath(`/api/rh/session/${encodeURIComponent(sessionId)}/preferences`), {
    method: "PATCH",
...
export async function finalizeInterviewSession(sessionId: string, preferredInputMode: InputMode) {
  const res = await fetch(rhApiPath(`/api/rh/session/${encodeURIComponent(sessionId)}/finalize`), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
