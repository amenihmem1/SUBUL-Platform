...
const historyTranslations = {
  fr: {
    ...
    help: "Help",
    title: "Historique",
    subtitle: "Historique des entretiens et conversations candidates.",
  ...
  en: {
    ...
    help: "Help",
    title: "History",
    subtitle: "Candidate interview and conversation history.",
...
export default function HistoryPage() {
  ...
  const pinnedSessions = historySessions.filter((session) => session.pinned).length;
  const archivedSessions = historySessions.filter((session) => session.archived).length;
  ...
  useEffect(() => {
    ...
    if (openTarget === "rh") {
      router.replace(rhHref);
    } else if (openTarget === "insights") {
      router.replace(insightsHref);
    }
  }, [historyLoading, insightsHref, latestSessionCompleted, latestSessionId, rhHref, router, searchParams]);
  ...
  }, [archivedSessions, historyLoading, pinnedSessions]);
  ...
  const updateSessionMeta = async (
    ...
    const res = await fetch(rhApiPath(`/api/rh/session/${encodeURIComponent(targetSessionId)}`), {
      method: "PATCH",
      ...
      <aside className={`${styles.sidebar} ${sidebarOpen ? styles.sidebarOpen : ""}`}>
        ...
        <div className={styles.menuBlock}>
          ...
          <nav className={styles.nav}>
            ...
            <Link className={`${styles.navItem} ${styles.navItemActive}`} href="/history" aria-current="page">
