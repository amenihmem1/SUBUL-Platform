    ...
    case "object":
      return (
        <svg viewBox="0 0 24 24" aria-hidden="true">
          ...
          <path d="M12 12 4.6 7.8" />
          <path d="M12 12l7.4-4.2" />
          <path d="M12 12v8.5" />
        </svg>
      );
    case "advice":
...
export function CandidateInsightsPopup({
  ...
  const providerTension = Math.max(
    0,
    ...providerEntries.map(([, metrics]) => {
      const sampleCount = Number((metrics as { samples?: number })?.samples || 0);
      return sampleCount > 0 ? Number((metrics as { tension_pct?: number })?.tension_pct || 0) : 0;
    }),
  );
    ...
    dominantHesitation === "noticeable"
      ...
      : dominantHesitation === "moderate"
        ...
        : dominantHesitation === "light"
          ? 4
          : 0;
  const derivedVisualStress = clampPercent(
    (100 - Number(visualMetrics?.looking_forward_pct || 0)) * 0.32 +
      (100 - Number(visualMetrics?.centered_pct || 0)) * 0.24 +
      Number(visualMetrics?.multiple_faces_pct || 0) * 0.44 +
      (visualFlags.includes("tension_apparente_moderee")
        ? 28
        : visualFlags.includes("tension_apparente_legere")
          ? 16
          : 0),
  );
  const derivedVocalStress = audioSampleLimited
    ? 0
    : clampPercent(
        Number(audioMetrics?.silence_pct_avg || 0) * 0.55 +
          Number(audioMetrics?.pause_count_avg || 0) * 2.2 +
          hesitationBase +
          vocalTensionBoost,
      );
  const derivedPauseStress = audioSampleLimited
    ? 0
    : clampPercent(Number(audioMetrics?.pause_rate_per_min_avg || 0) * 2.8 + Number(audioMetrics?.silence_pct_avg || 0) * 0.38);
  const derivedExpressionStress = clampPercent(
    Math.max(0, Number(visualMetrics?.neutral_pct || 0) - Number(visualMetrics?.smile_pct || 0)) * 0.42 +
      providerTension * 0.35,
  );
  const fallbackStressScoreValues = [
    derivedVisualStress,
    derivedExpressionStress,
    ...(audioSampleLimited ? [] : [derivedVocalStress, derivedPauseStress]),
  ];
  const fallbackStressScore = fallbackStressScoreValues.length
    ? clampPercent(fallbackStressScoreValues.reduce((sum, value) => sum + value, 0) / fallbackStressScoreValues.length)
    : clampPercent(providerTension + vocalTensionBoost);
  ...
  const apparentTension =
    typeof stressContext?.score === "number"
      ...
      : fallbackStressScore;
  const stressFactorsFromContext = (
    ...
    .slice(0, 4);
  const derivedStressFactors: StressFactor[] = [
    {
      key: "visual_stability",
      label: isEnglish ? "Visual stability" : "Stabilite visuelle",
      value: derivedVisualStress,
      detail: isEnglish ? "Derived from gaze, framing, and visual tension flags." : "Derive du regard, du cadrage et des signaux visuels.",
    },
    {
      key: "expression_balance",
      label: isEnglish ? "Expression balance" : "Equilibre expressif",
      value: derivedExpressionStress,
      detail: isEnglish ? "Estimated from neutral, smiling, and model tension cues." : "Estime depuis les expressions neutres, positives et la tension du modele.",
    },
    ...(audioSampleLimited
      ? []
      : [
          {
            key: "vocal_delivery",
            label: isEnglish ? "Vocal delivery" : "Lecture vocale",
            value: derivedVocalStress,
            detail: isEnglish ? "Derived from silences, pauses, and hesitation level." : "Derive des silences, pauses et hesitations.",
          },
          {
            key: "pause_load",
            label: isEnglish ? "Pause load" : "Charge de pauses",
            value: derivedPauseStress,
            detail: isEnglish ? "Estimated from pause rate and silence share." : "Estime depuis le taux de pauses et la part de silence.",
          },
        ]),
  ];
  const stressFactors = stressFactorsFromContext.length ? stressFactorsFromContext : derivedStressFactors.slice(0, 4);
  ...
  const detectedObjects = Array.isArray(visualMetrics?.detected_objects)
    ? visualMetrics.detected_objects
        ...
        .filter((item) => item.label && item.label.toLowerCase() !== "remote" && item.count > 0)
    : [];
  const objectTotal = detectedObjects.reduce((sum, item) => sum + item.count, 0);
                      ...
                      {detectedObjects.map((item) => (
                        <article className="candidate-insights-object-card" key={item.label}>
                          ...
                          <div>
                            ...
                            <span>{isEnglish ? `${item.count} times` : `${item.count} fois`}</span>
                          </div>
                        </article>
                      ))}
                    </div>
