...
const interviewTranslations = {
  fr: {
    ...
    meetingHeaderTitle: "HR Interview Assistant",
    meetingHeaderSubtitle:"Un espace de travail raffine pour lancer les entretiens, suivre chaque echange et finaliser le rapport candidat avec clarte.",
    session: "Session",
    ended: "Terminee",
    running: "En cours",
    active: "Active",
    ...
    voiceModePlaceholder: "Mode micro actif. Passez en mixte ou texte pour ecrire.",
    interviewDefaultPlaceholder: "Bonjour, je suis prete pour l'entretien.",
    cvRequiredPlaceholder: "Telechargez le CV avant de repondre.",
    cvRequiredMessage: "Veuillez telecharger le CV avant de commencer l'entretien.",
    ...
    controlSubtitle: "Telechargez le CV pour enrichir le contexte de l'entretien.",
    dragResume: "Glissez votre CV ici pour commencer",
  ...
  en: {
    ...
    meetingHeaderSubtitle:
      "A refined workspace to launch interviews, follow each exchange, and finalize the candidate report with clarity.",
    session: "Session",
    ended: "Ended",
    running: "Running",
    active: "Active",
    ...
    voiceModePlaceholder: "Microphone mode is active. Switch to mixed or text mode to type.",
    interviewDefaultPlaceholder: "Hello, I am ready for the interview.",
    cvRequiredPlaceholder: "Upload the CV before answering.",
    cvRequiredMessage: "Please upload the CV before starting the interview.",
    ...
    controlSubtitle: "Upload the resume to enrich the interview context.",
...
function HomePageContent() {
  ...
  const initialLocale = searchParams.get("locale") || searchParams.get("lang");
  const initialTheme = searchParams.get("theme");
  const [language, setLanguage] = useState<Language>(initialLocale === "en" ? "en" : "fr");
  const [hideLanguageSwitcher, setHideLanguageSwitcher] = useState(searchParams.get("hideLanguageSwitcher") !== "0");
  const [hideThemeSwitcher, setHideThemeSwitcher] = useState(searchParams.get("hideThemeSwitcher") !== "0");
  ...
  useEffect(() => {
    ...
    const platformTheme = searchParams.get("theme");
    const embeddedByPlatform = searchParams.get("hideLanguageSwitcher") === "1";
    const storedTheme = window.localStorage.getItem("report-dashboard-theme");
    if (platformTheme === "light" || platformTheme === "dark") {
      setTheme(platformTheme);
    } else if (!embeddedByPlatform && (storedTheme === "light" || storedTheme === "dark")) {
      setTheme(storedTheme);
    } else if (window.matchMedia("(prefers-color-scheme: dark)").matches) {
      setTheme("dark");
    }

    const platformLocale = searchParams.get("locale") || searchParams.get("lang");
    const storedLanguage = window.localStorage.getItem("dashboard-language");

    if (platformLocale === "fr" || platformLocale === "en") {
      setLanguage(platformLocale);
    } else if (!embeddedByPlatform && (storedLanguage === "fr" || storedLanguage === "en")) {
  ...
  useEffect(() => {
    ...
    setHideLanguageSwitcher(searchParams.get("hideLanguageSwitcher") !== "0");
    setHideThemeSwitcher(searchParams.get("hideThemeSwitcher") !== "0");
  }, [searchParams]);
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== "SUBUL_PLATFORM_LANGUAGE") return;
      ...
      if (typeof data.hideLanguageSwitcher === "boolean") {
        setHideLanguageSwitcher(data.hideLanguageSwitcher);
      }

      if (typeof data.hideThemeSwitcher === "boolean") {
        setHideThemeSwitcher(data.hideThemeSwitcher);
      }
    };
  ...
  const handleInputModeChange = async (nextMode: InputMode, options?: { persist?: boolean }) => {
    const normalizedMode = normalizeInputMode(nextMode);
    setInputMode(normalizedMode);

    if (normalizedMode === "text" && micListening) {
      await stopMic();
    }
  ...
  const hydrateSession = async (targetSessionId: string) => {
    try {
      ...
      setLoadingCv(false);
      setSending(false);
      setInputMode(normalizeInputMode(data?.preferred_input_mode));
  ...
  const speakFallbackAudio = (text: string, audioBase64?: string, audioMimeType?: string) => {
    ...
    void (async () => {
      try {
        ...
        const res = await fetch(rhApiPath("/api/rh/tts"), {
          method: "POST",
  ...
  const requireUploadedCv = () => {
    if (cvUploaded) {
      return true;
    }
    pushFeed("system", copy.cvRequiredMessage);
    return false;
  };
  ...
  const submitCandidateText = async (text: string, options?: { stopMicAfterSend?: boolean }) => {
    ...
    try {
      const res = await fetch(rhApiPath(`/api/rh/session/${encodeURIComponent(sessionId)}/message`), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const recordCandidateAudioObservation = async (payload: AudioObservationPayload) => {
    ...
    const res = await fetch(rhApiPath(`/api/rh/session/${encodeURIComponent(sessionId)}/audio`), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const transcribeRecordedUtterance = async (blob: Blob, samples?: Float32Array | null) => {
    if (!blob.size) {
      return;
    }
    ...
    try {
      const res = await fetch(rhApiPath("/api/rh/stt"), {
        method: "POST",
        body: form,
      });
      const data = await res.json();
  ...
  const startMic = async () => {
    ...
    try {
      ...
      micFinalizeUtteranceRef.current = async () => {
        ...
        await transcribeRecordedUtterance(blob, wavSamples);
      };
  ...
  const stopMic = async (options?: { submitPending?: boolean }) => {
    ...
    if (recordedBlob && recordedBlob.size > 0 && !sendingRef.current && !interviewEnded) {
      const pendingSamples = collectPendingMicObservationSamples();
      disposeLiveMic();
      await transcribeRecordedUtterance(recordedBlob, pendingSamples);
      return;
    }
  ...
  const startCamera = async () => {
    if (interviewEnded) {
      pushFeed("system", "The interview has been ended. Reload the page to open a new session.");
      return;
    }
    if (typeof window === "undefined" || !navigator.mediaDevices?.getUserMedia) {
      pushFeed("system", "Camera access is not supported in this browser.");
      return;
    }
    try {
      ...
      const msg = (error as Error)?.message || "";
      if (msg.toLowerCase().includes("denied") || msg.toLowerCase().includes("permission")) {
        pushFeed("system", "Camera access was denied. Enable it in your browser permissions.");
        return;
      }
      pushFeed("system", `Unable to start the camera: ${msg || "unknown error"}`);
    }
  };
  ...
  const onUploadCv = async () => {
    ...
    try {
      ...
      const res = await fetch(rhApiPath(`/api/rh/session/${encodeURIComponent(sessionId)}/cv`), {
        method: "POST",
        body,
      });
      const data = await res.json();
  ...
  const rhHref = `/report/${encodeURIComponent(sessionId)}?view=rh`;
  ...
  const insightsAvailable = cvUploaded || cameraEnabled || feed.length > 0;
  const [sidebarOpen, setSidebarOpen] = useState(false);

  return (
    <div
      className={`${styles.shell} ${sidebarOpen ? styles.sidebarVisible : ""} ${theme === "dark" ? styles.themeDark : styles.themeLight}`}
      data-theme={theme}
    >
      <button
        type="button"
            ...
            <section className="stage">
              <InterviewFeedPanel
                ...
                copy={{
                  ...
                  cvRequiredPlaceholder: copy.cvRequiredPlaceholder,
              ...
              <section className="meeting-grid">
                <CandidateTile
                  ...
                  cvUploaded={cvUploaded}
                  inputMode={inputMode}
                  interviewActive={interviewStartedAt !== null}
                  interviewEnded={interviewEnded}
