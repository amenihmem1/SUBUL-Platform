...
export default function CancelInterviewPage({ params }: CancelInterviewPageProps) {
  ...
  useEffect(() => {
    ...
    const resolveAndLoad = async () => {
      try {
        ...
        const response = await fetch(rhApiPath(`/api/rh/interviews/cancel/${encodeURIComponent(resolved.token || "")}`), {
          method: "GET",
          cache: "no-store",
        });
  ...
  const handleCancel = async () => {
    ...
    setSuccessMessage("");
    try {
      const response = await fetch(rhApiPath(`/api/rh/interviews/cancel/${encodeURIComponent(token)}`), {
        method: "POST",
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
