...
const translations = {
  fr: {
    ...
    resultsPendingMessage: "Le candidat doit terminer son entretien avant d'acceder aux vues RH et Insights.",
    ...
    rhReport: "Dashboard RH",
    visualVocalInsights: "Insights Visuels & Vocaux",
    rhSubtitle: "Vue RH consolidee du profil et des competences pour",
    ...
    closeModal: "Fermer",
    qrLocalHint: "Pour un scan sur telephone, configurez NEXT_PUBLIC_APP_URL avec une URL frontend accessible.",
    preparing: "Preparation...",
    candidateProfile: "Profil candidat",
    rhScore: "Score RH",
    skillsDistribution: "Niveaux des competences",
    topCvSkills: "Top competences CV",
    rhSummary: "Recapitulatif RH",
    rhSummarySubtitle: "Lecture detaillee des dimensions evaluees pendant l'entretien.",
    status: "Statut",
    dimension: "Dimension",
    score: "Score",
    keySignal: "Signal cle",
    recommendedAction: "Action recommandee",
    ...
    clearExpression: "Expression claire",
    goodCollaboration: "Bonne collaboration",
    analyticalAbility: "Capacite d'analyse",
    engagementToStrengthen: "Engagement a renforcer",
    hrReading: "Lecture RH",
    coherentProfile: "Le profil presente un niveau global coherent et exploitable pour la suite du processus RH.",
    attention: "Attention",
    noMajorAlert: "Aucun signal d'alerte majeur n'a ete detecte, mais la lecture doit rester prudente.",
    recommendation: "Recommandation",
    followupRecommendation: "Prevoir un echange complementaire pour approfondir les points de motivation et l'impact des exemples.",
  ...
  en: {
    ...
    resultsPendingMessage: "The candidate must finish the interview before accessing the HR and Insights views.",
    mainMenu: "Main menu",
    ...
    rhReport: "HR Dashboard",
    visualVocalInsights: "Visual & Vocal Insights",
    rhSubtitle: "Consolidated HR view of profile and competencies for",
    ...
    closeModal: "Close",
    qrLocalHint: "For phone scanning, set NEXT_PUBLIC_APP_URL to a frontend URL the device can reach.",
    preparing: "Preparing...",
    candidateProfile: "Candidate profile",
    rhScore: "HR Score",
    skillsDistribution: "Competency levels",
    topCvSkills: "Top CV skills",
    rhSummary: "HR Summary",
    rhSummarySubtitle: "Detailed reading of the dimensions assessed during the interview.",
    status: "Status",
    dimension: "Dimension",
    score: "Score",
    ...
    goodCollaboration: "Good collaboration",
    analyticalAbility: "Analytical ability",
    engagementToStrengthen: "Engagement to strengthen",
    hrReading: "HR reading",
    coherentProfile: "The profile shows a coherent overall level and is usable for the next HR steps.",
    attention: "Attention",
    noMajorAlert: "No major alert signal was detected, but the reading should remain cautious.",
    recommendation: "Recommendation",
    followupRecommendation: "Plan a follow-up exchange to explore motivation points and the impact of examples in more depth.",
...
const dynamicFrToEnMap = new Map<string, string>([
  ...
  ["Le profil presente un niveau global coherent et exploitable pour la suite du processus RH.", "The profile shows a coherent overall level and is usable for the next HR steps."],
  ...
  ["Prevoir un echange complementaire pour approfondir les points de motivation et l'impact des exemples.", "Plan a follow-up exchange to explore motivation points and the impact of examples in more depth."],
  ...
  ["Communication solide", "Strong communication"],
  ["Travail en equipe solide", "Strong teamwork"],
  ["Resolution de problemes solide", "Strong problem solving"],
]);
...
function translateDynamicTextV2(text: string | undefined, language: Language) {
  ...
  return applyTextReplacementsV2(raw, [
    ...
    ["Strong communication", "Communication solide"],
    ["Strong teamwork", "Travail en equipe solide"],
    ["Strong problem solving", "Resolution de problemes solide"],
  ]);
}

const competencyMeta = [
  { key: "communication", labelKey: "communication", color: "#7c3aed" },
  { key: "teamwork", labelKey: "teamwork", color: "#d946ef" },
  { key: "problem_solving", labelKey: "resolution", color: "#a855f7" },
  { key: "motivation", labelKey: "motivation", color: "#ec4899" },
] as const;
...
function normalizeEmailContact(value?: string) {
  const cleaned = String(value || "").trim();
  if (!cleaned || !cleaned.includes("@")) return cleaned;
  const [localPart, ...domainParts] = cleaned.split("@");
  const domain = domainParts.join("@");
  const normalizedLocal = localPart.replace(/^(?:(?:e-?mail|mail|linkedin|linkedln|linked-in|contact)[:\s._-]*)+/i, "");
  return normalizedLocal && domain ? `${normalizedLocal}@${domain}` : cleaned;
}
...
function SidebarIcon({ type }: { type: "calendar" | "dashboard" | "hire" | "file" | "help" | "settings" | "interview" | "memory" }) {
  if (type === "memory") {
    return (
      <svg viewBox="0 0 24 24" aria-hidden="true">
...
export default function ReportDashboardPage() {
  ...
  const [activeView, setActiveView] = useState<"rh" | "insights">("rh");
  ...
  useEffect(() => {
    ...
    const loadReport = async () => {
      ...
      setError("");
      try {
        const res = await fetch(rhApiPath(`/api/rh/session/${encodeURIComponent(sessionId)}?include_insights=1&language=${encodeURIComponent(language)}`), {
          method: "GET",
          cache: "no-store",
        });
  ...
  const isRhView = activeView === "rh";
  const isInsightsView = activeView === "insights";
  const rhPdfViewUrl = sessionId && shareOrigin ? `${shareOrigin}/report/${encodeURIComponent(sessionId)}/pdf` : "";
  const insightsPdfViewUrl =
    sessionId && shareOrigin
      ...
      : "";
  const activePdfViewUrl = isInsightsView ? insightsPdfViewUrl : rhPdfViewUrl;
  ...
  const liveInsightsPayload: CandidateInsightsPayload | null = payload
    ? {
        ...
        visual_context:
          ...
          (finalReport?.visual_metrics
            ? {
                metrics: finalReport.visual_metrics,
                signals: finalReport.visual_signals || [],
                heuristic_flags: finalReport.visual_flags || [],
                confidence_note: finalReport.confidence_note || "",
              }
            : null),
  ...
  const candidateEmail = normalizeEmailContact(payload?.cv_profile?.email);
  const contactItems = [
    {
      label: copy.email,
  ...
  const competencyCards = useMemo(() => {
    const scores = finalReport?.competencies || {};
    return competencyMeta.map((item) => {
      const rawValue = Number(scores[item.key] || 0);
      const scaled = clamp(rawValue * 20);
      return {
        ...item,
  ...
  const totalScore = Number(finalReport?.score_total || competencyCards.reduce((sum, item) => sum + item.scaled, 0) / 4 || 0);
  ...
  const topSkills = (payload?.cv_profile?.top_skills || []).slice(0, 4);
  const turns = payload?.turns || [];

  const kpis: KpiCard[] = [
    {
      label: copy.scoreGlobalRh,
    ...
    },
    {
      ...
      value: formatScore(competencyCards[0]?.rawValue || 0),
      helper: rawDyn((finalReport?.strengths || [])[0]) || copy.clearExpression,
      tone: toneFromScore((competencyCards[0]?.rawValue || 0) * 20),
    },
    {
      label: copy.teamwork,
      value: formatScore(competencyCards[1]?.rawValue || 0),
      helper: rawDyn((finalReport?.strengths || [])[1]) || copy.goodCollaboration,
      tone: toneFromScore((competencyCards[1]?.rawValue || 0) * 20),
    },
    {
      label: copy.resolution,
      value: formatScore(competencyCards[2]?.rawValue || 0),
      helper: rawDyn((finalReport?.strengths || [])[2]) || copy.analyticalAbility,
      tone: toneFromScore((competencyCards[2]?.rawValue || 0) * 20),
    },
    {
      label: copy.motivation,
      value: formatScore(competencyCards[3]?.rawValue || 0),
      helper: rawDyn((finalReport?.improvement_points || [])[0]) || copy.engagementToStrengthen,
      tone: toneFromScore((competencyCards[3]?.rawValue || 0) * 20),
    },
  ];

  const insightsItems = [
    {
      label: copy.hrReading,
      text:
  ...
  const proctoringEvents = (payload?.proctoring_events || finalReport?.proctoring_events || []).filter(
    (event) => event && (event.message || event.reason || event.time)
  );
  ...
  const skillBars = topSkills.map((skill, index) => ({
      label: skill.toUpperCase(),
      value: clamp(92 - index * 10, 40, 100),
  }));
  ...
  const donutSegments = useMemo(() => {
    const total = competencyCards.reduce((sum, item) => sum + (item.rawValue || 0), 0) || 1;
    let currentAngle = 0;

    return competencyCards.map((item) => {
      const rawValue = item.rawValue || 0;
      const sweep = (rawValue / total) * 360;
      ...
      return {
        ...item,
        rawValue,
        sharePercent: Math.round((rawValue / total) * 100),
        gradePercent: clamp(rawValue * 20),
  ...
  const detailRows = [
    ...competencyCards.map((item, index) => ({
      dimension: item.label,
      score: formatScore(item.rawValue || 0),
      signal: rawDyn(finalReport?.dimension_signals?.[item.key]) || rawDyn((finalReport?.strengths || [])[index]) || "",
      action: rawDyn(finalReport?.dimension_actions?.[item.key]) || "",
      progress: clamp((item.rawValue || 0) * 20),
      tone: toneFromScore((item.rawValue || 0) * 20),
    })),
  ];
  ...
  const insightSignalRows = [
    ...
    },
    {
      ...
      items:
        (payload?.audio_context?.signals || finalReport?.audio_signals || [copy.noVocalSignal]).map((item) => rawDyn(item) || copy.noVocalSignal),
    },
  ];
  ...
  const downloadPdf = async () => {
    ...
    try {
      const res = await fetch(rhApiPath(`/api/rh/session/${encodeURIComponent(sessionId)}/report`), {
        method: "GET",
      });
      if (!res.ok) {
      ...
      const fileName = fileNameMatch?.[1] || `${sessionId}-rapport-rh.pdf`;
  ...
  const downloadInsightsPdf = async () => {
    ...
    try {
      const res = await fetch(rhApiPath(`/api/rh/session/${encodeURIComponent(sessionId)}/insights-report?language=${encodeURIComponent(language)}`), {
        method: "GET",
      });
      if (!res.ok) {
        const raw = await res.text();
    ...
    try {
      ...
      if (typeof navigator !== "undefined" && navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
        await navigator.share({
          title: `${copy.rhReport} - ${candidateName}`,
          text: `${copy.rhReport} ${candidateName}`,
          files: [pdfFile],
        });
        return;
      }
        ...
        <main className={styles.main}>
          <section className={styles.loadingStage} aria-label={copy.loadingDashboard}>
            ...
            <div className={styles.loadingCard}>
              <div className={styles.loadingEyebrow}>SUBUL RH</div>
  ...
  return (
    <div className={`${styles.shell} ${sidebarOpen ? styles.sidebarVisible : ""} ${effectiveTheme === "dark" ? styles.themeDark : styles.themeLight}`}>
      <button
        type="button"
      ...
      <aside className={`${styles.sidebar} ${sidebarOpen ? styles.sidebarOpen : ""}`}>
        ...
        <div className={styles.menuBlock}>
          ...
          <nav className={styles.nav}>
            ...
            {reportUnlocked ? (
              <Link
                className={`${styles.navItem} ${isRhView ? styles.navItemActive : ""}`}
                href={`/report/${encodeURIComponent(sessionId)}?view=rh`}
                onClick={() => setActiveView("rh")}
                aria-current={isRhView ? "page" : undefined}
              >
        ...
        >
          {isRhView ? (
            <div>
              <h1>{copy.rhReport}</h1>
              <p>
                {copy.rhSubtitle} <strong>{candidateName}</strong>.
              </p>
            </div>
          ) : isInsightsView ? (
            <div>
        ...
        <section className={`${styles.sectionBlock} ${styles.rhSection}`}>
              ...
              <div className={styles.donutSection}>
                <div className={styles.donutRing}>
                  ...
                  <div className={styles.donutCenter}>
                    <strong>{activeCompetency ? formatPercent(activeCompetency.gradePercent) : Math.round(totalScore)}</strong>
                ...
                <div className={styles.legendList}>
                  {donutSegments.map((item, index) => (
                    ...
                    >
                      ...
                      <span>
                        {item.label} ({formatScore(item.rawValue || 0)}) - {formatPercent(item.gradePercent)}
                      </span>
                    </button>
                  ))}
                </div>
              ...
              <div className={styles.proctoringList}>
                {proctoringEvents.map((event, index) => (
                  <div className={styles.proctoringItem} key={`${event.time || event.reason || "event"}-${index}`}>
                    ...
                    <span>{[event.reason, event.time].filter(Boolean).join(" - ")}</span>
                  </div>
                ))}
              </div>
            ) : (
              <p className={styles.emptyState}>{copy.noProctoringAlerts}</p>
            )}
          </section>
          ...
          <section className={styles.tableCard} id="table">
            <div className={styles.tableHead}>
              <div>
                <h3>{copy.rhSummary}</h3>
                <p>{copy.rhSummarySubtitle}</p>
              </div>
                    ...
                    <tr key={row.dimension}>
                      ...
                      <td>
                        <div className={styles.progressCell}>
                          <strong>{formatPercent(row.progress)}</strong>
