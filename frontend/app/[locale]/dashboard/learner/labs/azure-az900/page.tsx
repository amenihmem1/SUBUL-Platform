...
import { FlaskConical, CheckSquare, ChevronRight, Eye, ArrowLeft, Target, Zap, Award, ExternalLink } from 'lucide-react'
...
const AZURE_THEME = {
  primary: '#0078D4',
  gradient: 'linear-gradient(135deg, #0078D4 0%, #00D4C2 100%)',
  bg: 'bg-blue-50',
  text: 'text-blue-700',
...
export default function AzureAZ900LabsPage() {
  const { t } = useTranslation()
  const router = useRouter()
  const pathname = usePathname()
  const locale = pathname.split('/')[1] || 'fr'
  const { data: course, isLoading, error } = useCourse(AZ900_COURSE_ID)
      ...
      <div className="py-4 sm:py-5 border-b border-border">
        <div className="flex flex-wrap items-center gap-3 mb-2">
          ...
          <div className="flex flex-wrap gap-2 text-sm">
            <span className={cn('inline-flex items-center gap-1.5 rounded-full px-3 py-1 font-semibold border', AZURE_THEME.bg, AZURE_THEME.text)} style={{ borderColor: AZURE_THEME.primary }}>
        ...
        <p className="text-sm text-muted-foreground">{t('learnerLabs.azureAz900Subtitle')}</p>
      </div>
              ...
              <motion.div key={lab.slug} variants={cardVariants} className="group">
                <Card
                  ...
                  style={{
                    ...
                    borderColor: AZURE_THEME.primary,
                  }}
                >
                  <div
                    className="absolute top-0 left-0 right-0 h-1 opacity-0 group-hover:opacity-100 transition-opacity"
                    style={{ background: AZURE_THEME.gradient }}
                  />
                  <CardHeader className="pb-4 sm:pb-6 relative z-10">
                    <div className="flex items-start gap-3 sm:gap-4">
                      <div
                        ...
                        style={{
                          background: AZURE_THEME.gradient,
                          boxShadow: `0 8px 32px ${AZURE_THEME.primary}20`,
                        }}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          <span
                            ...
                            style={{ borderColor: AZURE_THEME.primary }}
                          >
                            {isBeginner ? t('learnerLabs.beginner') : t('learnerLabs.intermediate')}
                          </span>
                        </div>
                        <CardTitle className="text-base sm:text-lg font-bold text-gray-900 leading-tight group-hover:text-blue-600 transition-colors">
                          {lab.title}
                        </CardTitle>
                  ...
                  <CardContent className="space-y-4 pt-5 relative z-10">
                    ...
                    <ul className="space-y-2">
                      ...
                      {lab.tasks.length > 3 && (
                        <li className="text-sm text-blue-600 font-medium pl-3.5">+{lab.tasks.length - 3} autres tâches…</li>
                      )}
                    </ul>
                    <Button
