...
import { Loader2, Plus, ChevronDown, ChevronRight, Save, AlertCircle, GripVertical } from 'lucide-react';
...
import type { AdminCourseDetail, AdminCourseModule, CreateCoursePayload } from '@/services/admin-courses';
import { COURSE_LEVELS, type CourseLevel } from '@/lib/constants/course-level';
import {
  CourseModuleEditor,
  emptyModule,
  parseLines,
...
function apiModulesToFormModules(modules: AdminCourseDetail['modules']): ModuleForm[] {
  ...
  return modules.map((mod) => ({
    ...
    lessons: (mod.lessons ?? []).map((l) => ({
      title: l.title ?? '',
      content: l.content ?? '',
      bullets: Array.isArray(l.bullets) ? l.bullets.join('\n') : '',
    })),
    labs: (mod.labs ?? []).map((lab) => ({
      title: lab.title ?? '',
      labId: lab.labId ?? '',
      objective: lab.objective ?? '',
      learningObjectives: Array.isArray(lab.learningObjectives) ? lab.learningObjectives.join('\n') : '',
      durationMinutes: lab.durationMinutes != null ? String(lab.durationMinutes) : '',
      difficultyLevel: lab.difficultyLevel ?? '',
    })),
  }));
}

function formModulesToPayload(modules: ModuleForm[]): AdminCourseModule[] {
...
function SortableModuleCard({ moduleId, children }: { moduleId: string; children: React.ReactNode }) {
  const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: moduleId });
  const style = { transform: CSS.Transform.toString(transform), transition };
  return (
    <div ref={setNodeRef} style={style} className="relative">
      <button
        type="button"
        className="absolute right-3 top-3 z-20 rounded-md border bg-background/90 p-1 text-muted-foreground hover:text-foreground"
        {...attributes}
        {...listeners}
...
export default function AdminEditCoursePage() {
  ...
  const { data: course, isLoading, isError, refetch, isFetching } = useAdminCourse(id);
  ...
  useEffect(() => {
    if (course && !initialized) {
      const mods = apiModulesToFormModules(course.modules);
      setCourseId(course.courseId);
      setTitle(course.title);
      setDescription(course.description ?? '');
      setLevel(course.level ?? '');
      setCertificationId(course.certificationId != null ? String(course.certificationId) : '');
      setModules(mods);
      setModuleIds(mods.map((_, i) => `module-${i + 1}-${Math.random().toString(36).slice(2, 9)}`));
      setModuleOpen(Object.fromEntries(mods.map((_, i) => [i, true])));
      setInitialized(true);
    }
  }, [course, initialized]);
  ...
  const addModule = () => {
    ...
    setModuleIds((ids) => [...ids, `module-${ids.length + 1}-${Math.random().toString(36).slice(2, 9)}`]);
    setModuleOpen((o) => ({ ...o, [modules.length]: true }));
  };
  const removeModule = (index: number) => {
    setModules((m) => m.filter((_, i) => i !== index));
    setModuleIds((ids) => ids.filter((_, i) => i !== index));
    setModuleOpen((o) => {
  ...
  const handleModuleDragEnd = (event: DragEndEvent) => {
    ...
    setModuleIds((prev) => arrayMove(prev, oldIndex, newIndex));
    setModuleOpen((prev) => {
      const ordered = Array.from({ length: moduleIds.length }, (_, idx) => prev[idx] !== false);
      const reordered = arrayMove(ordered, oldIndex, newIndex);
      return Object.fromEntries(reordered.map((value, idx) => [idx, value]));
    });
  };

  const buildPayload = (): Partial<CreateCoursePayload> => ({
    courseId: courseId.trim(),
    title: title.trim(),
    description: description.trim() || undefined,
    level: level.trim() || undefined,
    certificationId: certificationId ? parseInt(certificationId, 10) : undefined,
    modules: formModulesToPayload(modules),
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateCourse.mutateAsync({ id, payload: buildPayload() });
      ...
      const message =
        typeof raw === 'string'
          ...
          : Array.isArray(raw) && raw.length > 0
            ? raw[0]
            : String(t('adminCourse.saveError'));
      setError(message);
    }
  };
    ...
    return (
      <Card>
        <CardContent className="py-8 text-center space-y-4">
          <p className="text-destructive">{String(t('adminCourse.notFound'))}</p>
          <div className="flex flex-col sm:flex-row gap-2 justify-center">
            <Button variant="outline" type="button" onClick={() => refetch()} disabled={isFetching}>
              {String(t('common.retry'))}
            </Button>
            <Button variant="outline" asChild>
              <Link href={`/${locale}/dashboard/admin/courses`}>{String(t('adminCourse.backToCourses'))}</Link>
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      >
        <div className="relative z-10">
          <nav className="flex items-center gap-2 text-sm text-white/70 mb-4 bg-white/10 w-fit px-3 py-1 rounded-full backdrop-blur-sm border border-white/10">
            ...
            <span className="opacity-50">/</span>
            <span className="text-white font-medium">{String(t('adminCourse.editBreadcrumb', { title: title || course.title }))}</span>
          </nav>
          
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mt-2">
            <div>
              <h1 className="text-3xl font-bold tracking-tight">{String(t('adminCourse.editPageTitle'))}</h1>
              <p className="text-white/80 mt-1">{String(t('adminCourse.editSubtitle', { courseId }))}</p>
            </div>
            <div className="flex gap-4 p-1 bg-white/10 backdrop-blur-md rounded-xl border border-white/20">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 text-white/50">
                ...
                <div className="space-y-4">
                  {modules.length === 0 && (
                    <div className="text-center py-12 rounded-xl border border-dashed border-muted-foreground/30 bg-muted/10">
                      <p className="text-sm text-muted-foreground">{String(t('adminCourse.noModulesEdit'))}</p>
                      <Button type="button" variant="link" onClick={addModule} className="mt-2">{String(t('adminCourse.addNow'))}</Button>
                    </div>
                  )}
                  <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleModuleDragEnd}>
                    <SortableContext items={moduleIds} strategy={verticalListSortingStrategy}>
                      {modules.map((mod, modIndex) => {
                        ...
                        const moduleKey = moduleIds[modIndex] ?? `module-${modIndex}`;
                        return (
                          <SortableModuleCard key={moduleKey} moduleId={moduleKey}>
                            <motion.div
                              layout
                              initial={{ opacity: 0, scale: 0.98 }}
                              animate={{ opacity: 1, scale: 1 }}
                              className="rounded-xl border border-border bg-white dark:bg-slate-900 overflow-hidden shadow-sm"
                            >
                              <button
                                type="button"
          ...
          <div className="space-y-6">
            <Card className="sticky top-6 shadow-md border-primary/20 overflow-hidden">
              ...
              <CardContent className="space-y-4">
                ...
                >
                  {updateCourse.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Save className="h-4 w-4 mr-2" />}
                  {String(t('adminCourse.saveCourse'))}
                </Button>
                <Button 
                  type="button" 
                  variant="outline" 
            ...
            <Card className="bg-muted/30 border-dashed">
              <CardHeader>
                <CardTitle className="text-sm font-semibold">{String(t('adminCourse.systemInfo'))}</CardTitle>
              </CardHeader>
              <CardContent className="text-xs text-muted-foreground space-y-2">
                <p>• {String(t('adminCourse.internalId'))}: {id}</p>
                <p>• {String(t('adminCourse.modulesCount'))}: {modules.length}</p>
                <p>• {String(t('adminCourse.certification'))}: {certificationId ? String(t('adminCourse.certLinked')) : String(t('adminCourse.certNone'))}</p>
              </CardContent>
            </Card>
          </div>
        </div>
