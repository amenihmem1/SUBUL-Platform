...
import { useLearnerCatalogCourses } from '@/hooks/api/useCourses';
...
export function ActiveCoursesSection() {
  ...
  const { data: catalogCourses = [], isLoading: isCatalogLoading } = useLearnerCatalogCourses(true);
  const activeCourses = data?.activeCourses ?? [];
  const pathname = usePathname();
  const locale = pathname.split('/')[1] || 'fr';

  if (isLoading || (activeCourses.length === 0 && isCatalogLoading)) {
    return (
  ...
  if (activeCourses.length === 0) {
    const recommendedCourses = catalogCourses.slice(0, 3);
        ...
        {recommendedCourses.length > 0 ? (
          <div className="grid gap-3 md:grid-cols-3">
            {recommendedCourses.map((course) => (
              <Link
                key={course.id}
                href={`/${locale}/dashboard/learner/cours/${course.id}`}
                className="group flex min-h-[150px] flex-col justify-between rounded-2xl border border-violet-100 bg-gradient-to-b from-violet-50 to-white p-4 transition-all hover:-translate-y-0.5 hover:border-violet-200 hover:shadow-md"
              >
                <div>
                  ...
                  <p className="text-sm font-bold leading-snug text-slate-950 line-clamp-2">
                    {course.title}
                  </p>
                  <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-slate-500">
                    {course.description || course.nextLesson}
                  </p>
                </div>
                <div className="mt-4 flex items-center justify-between text-xs">
                  <span className="font-medium text-slate-500">{course.duration}</span>
          ...
          <div className="rounded-2xl border border-dashed border-violet-200 bg-gradient-to-b from-slate-50 to-white px-6 py-14 text-center">
            ...
            <p className="mb-1 text-base font-semibold text-slate-900">No courses available yet</p>
