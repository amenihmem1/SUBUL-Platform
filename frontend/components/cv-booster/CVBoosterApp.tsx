...
// Ne pas forcer la redirection vers le login pour le workflow d'upload CV.
// cf. lueurs de debug : 401 devrait être traité comme erreur de contenu, pas une page d'auth automatique.
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/platform-data/${userId}`, {
      ...
      .then(async (r) => {
        if (r.status === 401) {
          setPlatformStatus("unauthorized");
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (res.status === 401) {
        setErrorMsg("Unauthorized: please login to continue.");
        setPhase("error");
        return;
      }
  ...
  const handleSave = useCallback(async (extraData: Record<string, unknown> = {}) => {
    ...
    try {
      ...
      queryClient.setQueryData(cvKeys.status(), {
        hasCv: true,
        status: "ready",
        fileName: fileName || pendingFile.name || null,
      });
  ...
  }, [pendingFile, selections, platformLabs, platformCerts, platformQuiz, userId, queryClient, fileName]);
        ...
        {phase === "idle" && (
          <div className="screen screen-idle">
            ...
            >
              ...
              <p style={{ fontSize: 12, color: "var(--text-faint)", margin: "-2px 0 2px" }}>
                ...
                <button
                  ...
                  onClick={() => {
                    const locale = typeof window !== "undefined"
                      ...
                      : "en";
                    window.location.href = `/${locale}/search-job?user_id=${userId}&scan=1`;
                  }}
                  style={{
        ...
        {phase === "error" && (
          <div className="screen screen-error">
            ...
            <div className="error-hint">
              The CV agent may still be starting up — wait a few seconds and try again.
              If the issue persists, check that all Docker services are running.
            </div>
