...
function HomePageContent() {
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch("/api/rh/sessions?limit=80", { method: "GET", cache: "no-store" });
      await res.json();
    } catch (error) {
  ...
  const speakFallbackAudio = (text: string, audioBase64?: string, audioMimeType?: string) => {
    ...
    void (async () => {
      try {
        ...
        const res = await fetch("/api/rh/tts", {
          method: "POST",
  ...
  const submitCandidateText = async (text: string, options?: { stopMicAfterSend?: boolean }) => {
    ...
    try {
      const res = await fetch(`/api/rh/session/${encodeURIComponent(sessionId)}/message`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const recordCandidateAudioObservation = async (payload: AudioObservationPayload) => {
    ...
    const res = await fetch(`/api/rh/session/${encodeURIComponent(sessionId)}/audio`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const transcribeRecordedUtterance = async (blob: Blob, samples?: Float32Array | null) => {
    ...
    try {
      const res = await fetch("/api/rh/stt", {
        method: "POST",
        body: form,
      });
      const data = await res.json();
  ...
  const onUploadCv = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/rh/session/${encodeURIComponent(sessionId)}/cv`, {
        method: "POST",
        body,
      });
      const data = await res.json();
