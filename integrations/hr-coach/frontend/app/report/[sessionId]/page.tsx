...
export default function ReportDashboardPage() {
  ...
  useEffect(() => {
    ...
    const loadReport = async () => {
      ...
      setError("");
      try {
        const res = await fetch(`/api/rh/session/${encodeURIComponent(sessionId)}?include_insights=1&language=${encodeURIComponent(language)}`, {
          method: "GET",
          cache: "no-store",
        });
  ...
  const downloadPdf = async () => {
    ...
    try {
      const res = await fetch(`/api/rh/session/${encodeURIComponent(sessionId)}/report`, {
        method: "GET",
      });
      if (!res.ok) {
  ...
  const downloadInsightsPdf = async () => {
    ...
    try {
      const res = await fetch(`/api/rh/session/${encodeURIComponent(sessionId)}/insights-report?language=${encodeURIComponent(language)}`, {
        method: "GET",
      });
      if (!res.ok) {
        const raw = await res.text();
