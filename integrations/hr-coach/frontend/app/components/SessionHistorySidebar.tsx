...
function dedupeSessionsById(sessions: SessionHistoryEntry[]) {
  ...
  sessions.forEach((session) => {
    ...
    if (!existing || String(session.updated_at || "") > String(existing.updated_at || "")) {
...
function buildSections(
  ...
  regular.forEach((session) => {
    const key = buildDateSectionKey(session.updated_at);
...
function SessionRow({
  ...
  onTogglePinned: (sessionId: string, nextPinned: boolean) => void | Promise<void>;
  onToggleArchived: (sessionId: string, nextArchived: boolean) => void | Promise<void>;
  onDelete: (entry: SessionHistoryEntry) => void | Promise<void>;
}) {
...
export function SessionHistorySidebar({
  ...
  const visibleSessionIdSet = useMemo(() => new Set(visibleSessions.map((session) => session.session_id)), [visibleSessions]);
  const selectedVisibleSessionIds = useMemo(
    () => Array.from(new Set(selectedSessionIds)).filter((sessionId) => visibleSessionIdSet.has(sessionId)),
    [selectedSessionIds, visibleSessionIdSet],
  );
  const selectedCount = selectedVisibleSessionIds.length;
  ...
  const toggleSessionSelected = (sessionId: string) => {
    if (!visibleSessionIdSet.has(sessionId)) return;
    setSelectedSessionIds((prev) =>
  ...
  const deleteSelectedSessions = () => {
    const selectedIdSet = new Set(selectedVisibleSessionIds);
    const selectedSessions = visibleSessions.filter((session) => selectedIdSet.has(session.session_id));
                    ...
                    <div className="history-chat-list">
                      {section.sessions.map((session) => (
                        <SessionRow
                          ...
                          onDelete={(entry) => onDeleteSessions([entry])}
                        />
                      ))}
                    </div>
                  </section>
