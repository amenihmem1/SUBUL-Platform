...
class LangChainJSONClient:
    def __init__(
        ...
        *,
        ...
        self.api_version = (api_version or "2024-02-15-preview").strip()
        self.provider = (provider or "openai_compatible").strip().lower()
        self.is_azure = self.provider in {"azure", "azure_openai"} or ".openai.azure.com" in self.base_url
        if self.is_azure:
    ...
    def healthcheck(self) -> dict[str, Any]:
        ...
        if self.is_azure:
            ...
            try:
                deployments = self._list_azure_deployments()
                info["ok"] = True
                info["available_models"] = [item for item in deployments if item]
                info["model_available"] = not deployments or self.model in info["available_models"]
                return info
            except Exception as exc:
                logger.info("Azure deployments healthcheck unavailable, falling back to chat probe: %s", exc)
            try:
                probe = self.client.invoke("Reponds uniquement: OK")
                probe_text = self._response_text(probe).strip().upper()
                info["ok"] = bool(probe_text)
                info["available_models"] = [self.model]
                info["model_available"] = True
                info["probe"] = probe_text[:20]
                return info
            except Exception as exc:
                info["error"] = str(exc)
                return info
    ...
    def _list_azure_deployments(self) -> list[str]:
        response = httpx.get(
    ...
    def _build_chain(self, *, temperature: float, max_tokens: int):
        ...
        model = self.client.bind(
            temperature=temperature,
            max_tokens=max_tokens,
            **({} if self.is_azure else {"extra_body": {"reasoning_effort": "low", "include_reasoning": False}}),
        )
        return (
