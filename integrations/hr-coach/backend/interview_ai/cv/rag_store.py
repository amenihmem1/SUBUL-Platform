...
class CVRAGStore:
    ...
    def ingest_cv(self, session_id: SessionId, filename: str, content: bytes) -> dict[str, Any]:
        ...
        index = self._build_index(embeddings) if embeddings is not None else None
    ...
    def retrieve_context(self, session_id: SessionId, query: str, top_k: int = 5) -> list[str]:
        ...
        if session.get("index") is None or session.get("embeddings") is None:
            return self._lexical_retrieve(chunks, query, top_k)
    ...
    def _embed_chunks(self, chunks: list[str]):
        self._ensure_model()
        if self.model is None or np is None:
            logger.warning("Semantic CV embeddings disabled; using lexical retrieval fallback.")
            return None
    ...
    def _ensure_model(self) -> None:
        ...
        try:
            ...
            logger.warning("Semantic CV model unavailable; using lexical retrieval fallback: %s", exc)
            self.model = None
            self.dim = None
    ...
    def _lexical_retrieve(self, chunks: list[str], query: str, top_k: int) -> list[str]:
        query_tokens = self._tokenize(query)
        query_lower = query.lower()
        scored: list[tuple[float, str]] = []
        for chunk in chunks:
            score = self._lexical_score(query_tokens, chunk)
            score += self._phrase_score(query, chunk)
            score += self._domain_bonus(query_lower, chunk.lower())
