...
from orchestrator.rh_workflow_support import (
...
class RHWorkflowState(TypedDict, total=False):
    session_id: str
    text: str
    candidate_name: str
    ...
    should_clarify: bool
    generated_output: RHAgentOutput
    final_output: RHAgentOutput
    latest_scores: dict[str, int] | None
    response_language: Literal["fr", "en"]


class LangGraphRHOrchestrator(RHWorkflowSupport):
    """LangGraph-backed RH workflow that preserves the current public API."""

    def __init__(self, intelligence, tts, session_store=None, cv_rag_store=None, cv_blob_storage=None):
        super().__init__(
            intelligence=intelligence,
            tts=tts,
            session_store=session_store,
            cv_rag_store=cv_rag_store,
            cv_blob_storage=cv_blob_storage,
        )
        self._graph = self._build_graph()

    @staticmethod
    def _can_rephrase(session: RHSessionState) -> bool:
        ...
        return bool(question and phase in RHWorkflowSupport.PHASE_SEQUENCE)

    def _build_graph(self):
        workflow = StateGraph(RHWorkflowState)
    ...
    def _node_load_session(self, state: RHWorkflowState) -> RHWorkflowState:
    ...
    @staticmethod
    def _route_after_load(state: RHWorkflowState) -> str:
    ...
    def _node_return_existing_final(self, state: RHWorkflowState) -> RHWorkflowState:
        return {"final_output": self._build_final_output(state["session"])}

    def _node_rephrase(self, state: RHWorkflowState) -> RHWorkflowState:
    ...
    def _node_retrieve_context(self, state: RHWorkflowState) -> RHWorkflowState:
    ...
    def _node_generate_payload(self, state: RHWorkflowState) -> RHWorkflowState:
    ...
    def _node_validate_output(self, state: RHWorkflowState) -> RHWorkflowState:
        current_phase = state["current_phase"]
        ...
        if output.phase == "FINAL" and len(state["session"].turns) < state["session"].interview_max_questions:
            output = output.model_copy(
                update={
                    "phase": current_phase,
                    "question_index": state["session"].last_question_index + 1,
                    "say": output.say or FALLBACK_RETRY_MESSAGE,
                    "final_report": None,
                }
            )
    ...
    @staticmethod
    def _route_after_validation(state: RHWorkflowState) -> str:
        output = state["generated_output"]
        return "ensure_final_report" if output.phase == "FINAL" else "persist_turn"

    @staticmethod
    def _route_after_persist(state: RHWorkflowState) -> str:
        output = state["generated_output"]
        return "speak_turn" if output.say else "end"

    def _node_persist_turn(self, state: RHWorkflowState) -> RHWorkflowState:
        output = state["generated_output"]
        ...
        logger.info(
            "Tour RH LangGraph | session=%s | phase=%s | rag=%.0fms | llm=%.0fms | persist=%.0fms | say=%d chars",
            state["session_id"],
            output.phase,
    ...
    def _node_speak_turn(self, state: RHWorkflowState) -> RHWorkflowState:
        output = state["generated_output"]
        if output.say:
            self.tts.speak(output.say)
        return {"final_output": output}

    def _node_ensure_final_report(self, state: RHWorkflowState) -> RHWorkflowState:
        session = state["session"]
        ...
        logger.info(
            "Tour RH LangGraph | session=%s | phase=FINAL | rag=%.0fms | llm=%.0fms | final_report=yes",
            state["session_id"],
            float(state.get("rag_duration_ms", 0.0) or 0.0),
            float(state.get("llm_duration_ms", 0.0) or 0.0),
        )
    ...
    def _node_build_final_output(self, state: RHWorkflowState) -> RHWorkflowState:
        session = state["session"]
    ...
    @staticmethod
    def _route_after_build_final_output(state: RHWorkflowState) -> str:
        output = state["final_output"]
        return "speak_final_output" if output.say else "end"

    def _node_speak_final_output(self, state: RHWorkflowState) -> RHWorkflowState:
        output = state["final_output"]
        if output.say:
            self.tts.speak(output.say)
        return {"final_output": output}
    ...
    def handle_candidate_text(
        ...
        if isinstance(output, RHAgentOutput):
            return output
        raise RuntimeError("LangGraph workflow completed without producing an RHAgentOutput.")
