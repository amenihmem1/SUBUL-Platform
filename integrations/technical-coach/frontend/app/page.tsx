...
const interviewTranslations = {
  fr: {
    ...
    meetingHeaderSubtitle: "Un espace de travail raffiné pour lancer les entretiens techniques, suivre chaque échange et finaliser le rapport candidat avec clarté.",
    session: "Session",
    ended: "Terminée",
    running: "En cours",
    active: "Active",
    finalizedByUser: "Finalisée par l'utilisateur",
    interviewStarted: "Entretien démarré",
    waitingLaunch: "En attente de lancement",
    resume: "CV",
    uploaded: "Téléchargé",
    notUploaded: "Non téléchargé",
    fileSelected: "Fichier sélectionné",
    uploadCvToStart: "Téléchargez un CV pour commencer",
    report: "Rapport",
    available: "Disponible",
    inProgress: "En cours",
    readyForDownload: "Prêt au téléchargement",
    generatedAtEnd: "Généré à la fin",
    timer: "Chrono",
    finalDuration: "Durée finale",
    liveInterviewClock: "Entretien en direct",
    startsAtFirstReply: "Démarre à la première réponse",
    ...
    interviewEndedMessage: "Entretien terminé",
    interviewEndedDescription: "La session a été fermée depuis les contrôles de la candidate.",
    interviewReadyDescription: "Téléchargez un CV, puis envoyez le premier message pour lancer l'entretien.",
    liveTranscriptLabel: "Transcription en direct",
    candidateMessage: "Message candidate",
    interviewEndedPlaceholder: "Entretien terminé. Rechargez la page pour démarrer une nouvelle session.",
    voiceModePlaceholder: "Mode micro actif. Les réponses se font uniquement à la voix.",
    interviewDefaultPlaceholder: "Bonjour, je suis prête pour l'entretien.",
    ...
    controlSubtitle: "Téléchargez le CV, puis terminez l'entretien pour générer le rapport technique.",
...
function HomePageContent() {
  ...
  useEffect(() => {
    ...
    const reportProctoringViolation = (reason: ProctoringReason) => {
      ...
      const message =
        language === "fr"
          ? `Alerte surveillance ${count}: ${reasonLabel} détecté pendant l'entretien.`
  ...
  const startMic = async () => {
    ...
    try {
      ...
      processorNode.onaudioprocess = (event) => {
        ...
        if (micUtteranceActiveRef.current) {
          micUtteranceChunksRef.current.push({ samples: new Float32Array(samples), at: Date.now() });
        }
      };
      ...
      <main className={styles.main}>
        ...
        {captureShieldVisible && (
          <div className="capture-shield" role="alert" aria-live="assertive">
            ...
            <span>
              {language === "fr"
                ? "Les captures, impressions et copies sont bloquées pendant l'entretien."
