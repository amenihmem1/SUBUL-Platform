...
def _clean_contact_email(value: Any) -> str:
    ...
    local_part = re.sub(r"^(?:linkedin|e-?mail|courriel)[:._-]*", "", local_part, flags=re.IGNORECASE)
    return f"{local_part}@{domain}" if local_part else email
...
def normalize_cv_profile(profile: Any) -> dict[str, Any]:
    ...
    email = _clean_contact_email(raw.get("email"))
