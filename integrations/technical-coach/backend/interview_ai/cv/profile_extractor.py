...
ATTACHED_EMAIL_LABEL_RE = re.compile(r"^(?:linkedin|e-?mail|courriel)[:._-]*(.+)$", re.IGNORECASE)
...
def _extract_email(text: str) -> str:
    for line in text.splitlines():
        if "@" not in line:
            continue
        exact = _extract_first(EMAIL_PATTERN, line)
        if exact and ATTACHED_EMAIL_LABEL_RE.match(exact.split("@", 1)[0]):
...
def _normalize_email_candidate(candidate: str) -> str:
    ...
    attached_label = ATTACHED_EMAIL_LABEL_RE.match(local_part)
    if attached_label:
        local_part = attached_label.group(1)
