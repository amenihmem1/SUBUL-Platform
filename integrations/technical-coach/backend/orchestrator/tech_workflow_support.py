...
def collect_answer_scores(turns: list[dict[str, Any]]) -> dict[str, list[int]]:
...
def merge_scores(turns: list[dict[str, Any]]) -> dict[str, int]:
    collected = collect_answer_scores(turns)
    merged: dict[str, int] = {}
...
class TechnicalWorkflowSupport:
    ...
    def _get_or_create_session(self, session_id: str) -> TechSessionState:
        if session_id in self.sessions:
            ...
            if loaded:
                self._refresh_shared_media_state(state, loaded)
            return state
    ...
    def _refresh_shared_media_state(session: TechSessionState, loaded: dict[str, Any]) -> None:
        ...
        if isinstance(loaded.get("visual_observations"), dict):
            session.visual_observations = dict(loaded["visual_observations"])
        if isinstance(loaded.get("proctoring_events"), list):
            session.proctoring_events = list(loaded["proctoring_events"])
    ...
    def _build_final_report(
        ...
        self._backfill_missing_turn_scores(session, response_language=response_language)
        scored_turns = (
            [*session.turns, {"score_partial": latest_scores}]
            if isinstance(latest_scores, dict)
            else session.turns
        )
        competencies = merge_scores(scored_turns)
        ...
        collected_scores = collect_answer_scores(scored_turns)
        exact_scores = [value for values in collected_scores.values() for value in values]
        if exact_scores:
            global_score = round((sum(exact_scores) / (len(exact_scores) * 5)) * 100)
        else:
    ...
    def _score_candidate_answer(
        ...
        *,
        ...
        try:
            payload = self.intelligence.score_interview_turn(
                cv_profile=session.cv_profile,
                recent_turns=recent_turns if recent_turns is not None else session.turns,
                question=str(question or "").strip(),
                answer=cleaned_answer,
                question_phase=str(question_phase or "").strip().upper() or "QUESTION_1",
                response_language=response_language,
            )
            return normalize_score_partial(payload.get("score_partial") if isinstance(payload, dict) else {})
        except Exception as exc:
            logger.warning("Echec scoring reponse candidat", exc_info=exc)
            return empty_scores()

    def _backfill_missing_turn_scores(
        self,
        session: TechSessionState,
        *,
        response_language: Literal["fr", "en"],
    ) -> None:
        for index in range(1, len(session.turns)):
            turn = session.turns[index]
            previous_turn = session.turns[index - 1]
            if not isinstance(turn, dict) or not isinstance(previous_turn, dict):
                continue
            if has_meaningful_score(normalize_score_partial(turn.get("score_partial"))):
                continue

            turn["score_partial"] = self._score_candidate_answer(
                session,
                question=str(previous_turn.get("say", "") or ""),
                answer=str(turn.get("candidate_text", "") or ""),
                question_phase=str(previous_turn.get("phase", "") or ""),
                response_language=response_language,
                recent_turns=session.turns[:index],
            )

    def _score_latest_candidate_answer(
        self,
        session: TechSessionState,
        *,
        answer: str,
        ...
        previous_turn = session.turns[-1] if session.turns and isinstance(session.turns[-1], dict) else {}
        return self._score_candidate_answer(
            session,
            question=str(previous_turn.get("say", "") or ""),
            answer=answer,
            question_phase=str(previous_turn.get("phase", "") or ""),
    ...
    def _build_final_agent_output(
        ...
        *,
        lang: Literal["fr", "en"],
        score_partial: dict[str, int] | None = None,
    ) -> TechAgentOutput:
        return TechAgentOutput(
            ...
            score_partial=score_partial or empty_scores(),
    ...
    def _finalize_interview(
        ...
        if not has_meaningful_score(latest_scores):
            latest_scores = self._score_latest_candidate_answer(
                session,
                answer=last_text,
                response_language=lang,
            )
        self._ensure_final_report(session, latest_scores)
        output = self._build_final_agent_output(session, lang=lang, score_partial=latest_scores)
