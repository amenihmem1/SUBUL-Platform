...
def _merge_proctoring_events(*sources: object) -> list[dict[str, Any]]:
...
class JsonSessionStore(SessionStore):
    ...
    def save(self, session_id: str, payload: dict[str, Any]) -> None:
        ...
        payload_with_meta["proctoring_events"] = _merge_proctoring_events(
            existing.get("proctoring_events"),
            payload_with_meta.get("proctoring_events"),
        )
        if isinstance(payload_with_meta.get("final_report"), dict):
            payload_with_meta["final_report"]["proctoring_events"] = payload_with_meta["proctoring_events"]
            payload_with_meta["final_report"]["proctoring_alerts_count"] = len(payload_with_meta["proctoring_events"])
...
class PostgresSessionStore(SessionStore):
    ...
    def save(self, session_id: str, payload: dict[str, Any]) -> None:
        ...
        normalized_payload["proctoring_events"] = _merge_proctoring_events(
            existing.get("proctoring_events"),
            normalized_payload.get("proctoring_events"),
        )
        if isinstance(normalized_payload.get("final_report"), dict):
            normalized_payload["final_report"]["proctoring_events"] = normalized_payload["proctoring_events"]
            normalized_payload["final_report"]["proctoring_alerts_count"] = len(normalized_payload["proctoring_events"])
        now_iso = datetime.utcnow().isoformat() + "Z"
        existing_created = str(existing.get("created_at", "") or "").strip()
