...
def _draw_metric_panel(
    ...
    *,
    ...
    if icon:
        c.setFillColor(accent)
        c.circle(x + 22, y - 19, 8, fill=1, stroke=0)
        c.setFillColor(colors.white)
        c.setFont("Helvetica-Bold", 8)
        c.drawCentredString(x + 22, y - 22, icon[:2])
        title_x = x + 36
    else:
        title_x = x + 16
    c.setFillColor(COLOR_INK)
    c.setFont("Helvetica-Bold", 12)
    c.drawString(title_x, y - 22, title)
...
def _draw_report_header(
    ...
    c.setFillColor(COLOR_MUTED)
    c.setFont("Helvetica", 10)
    c.drawString(PAGE_MARGIN, separator_y - 22, f"{labels['date']} : {updated_at}")
    return y - 94


def _draw_report_score_card(
    c: canvas.Canvas,
    report: Dict[str, Any],
    x: float,
    y: float,
    width: float,
    labels: Dict[str, Any],
    score_total_override: int | None = None,
) -> float:
    score_total = int(score_total_override if score_total_override is not None else report.get("score_total", 0) or 0)
    c.setFillColor(COLOR_RH_SURFACE)
...
def _score_total_from_question_rows(rows: List[Dict[str, Any]], report: Dict[str, Any]) -> int:
    if rows:
        scores = [float(row.get("score", 0) or 0) for row in rows]
        return max(0, min(100, int(round((sum(scores) / max(1, len(scores))) * 20))))
    return max(0, min(100, int(report.get("score_total", 0) or 0)))
...
def build_candidate_report_pdf(session_id: str, payload: Dict[str, Any], output_dir: str | Path | None = None) -> Path:
    ...
    effective_score_total = _score_total_from_question_rows(question_grade_rows, report)

    c = canvas.Canvas(str(out_path), pagesize=A4)
    y = _draw_report_header(c, session_id, updated_at, candidate_name, headline, labels)
    y = _draw_report_score_card(c, report, PAGE_MARGIN, y, CONTENT_WIDTH, labels, effective_score_total)
